...
class PostLorittaPartnersInviteGuildDashboardRoute(website: LorittaDashboardWebServer) : RequiresGuildAuthDashboardLocalizedRoute(website, "/loritta-partners/invite") {
    @Serializable
    ...
    override suspend fun onAuthenticatedGuildRequest(call: ApplicationCall, i18nContext: I18nContext, session: LorittaUserSession, userPremiumPlan: UserPremiumPlans, theme: ColorTheme, shimejiSettings: LorittaShimejiSettings, guild: Guild, guildPremiumPlan: ServerPremiumPlans, member: Member) {
        ...
        if (request.inviteId == null) {
            call.respondHtmlFragment {
                div {
                    text(i18nContext.get(DashboardI18nKeysData.LorittaPartners.NoInviteConfigured))
                }
            }
            return
        }
        ...
        if (inviteCode == null || !DiscordInviteUtils.inviteCodeRegex.matches(inviteCode)) {
            call.respondHtmlFragment {
                div {
                    text(i18nContext.get(DashboardI18nKeysData.LorittaPartners.InvalidInviteUrl))
                }
            }
            return
        }
        ...
        val invite = try {
            ...
            call.respondHtmlFragment {
                div {
                    text(i18nContext.get(DashboardI18nKeysData.LorittaPartners.InvalidInvite))
                }
            }
            return
        }

        if (invite.guild?.idLong != guild.idLong) {
            call.respondHtmlFragment {
                div {
                    text(i18nContext.get(DashboardI18nKeysData.LorittaPartners.InvalidInviteWrongGuild))
                }
            }
            return
        }
