...
class DropsGuildDashboardRoute(website: LorittaDashboardWebServer) : RequiresGuildAuthDashboardLocalizedRoute(website, "/drops") {
    override suspend fun onAuthenticatedGuildRequest(call: ApplicationCall, i18nContext: I18nContext, session: LorittaUserSession, userPremiumPlan: UserPremiumPlans, theme: ColorTheme, shimejiSettings: LorittaShimejiSettings, guild: Guild, guildPremiumPlan: ServerPremiumPlans, member: Member) {
        val dropsConfig = website.loritta.transaction {
            DropsConfigs.selectAll()
                .where {
                    DropsConfigs.id eq guild.idLong
                }
                .firstOrNull()
        }
        ...
        call.respondHtml {
            dashboardBase(
                i18nContext,
                i18nContext.get(DashboardI18nKeysData.Drops.Title),
                session,
                theme,
                shimejiSettings,
                userPremiumPlan,
                ...
                {
                    guildDashLeftSidebarEntries(i18nContext, guild, userPremiumPlan, GuildDashboardSection.LORITTA_DROPS)
                },
                {
                    rightSidebarContentAndSaveBarWrapper(
                        website.shouldDisplayAds(call, userPremiumPlan, null),
                        {
                            ...
                            heroWrapper {
                                heroText {
                                    h1 {
                                        text(i18nContext.get(I18nKeysData.Website.Dashboard.Drops.Title))
                                    }

                                    for (str in i18nContext.language
                                        .textBundle
                                        .lists
                                        .getValue(I18nKeys.Website.Dashboard.Drops.Description.key)
                                    ) {
                                        p {
                                            handleI18nString(
                                                str,
                                                    ...
                                                    "dropCommand" -> {
                                                        TextReplaceControls.ComposableFunctionResult {
                                                            span(classes = "discord-mention") {
                                                                text("/${i18nContext.get(I18nKeysData.Commands.Command.Drop.Label)}")
                                                            }
                                                        }
                                                    }
                            ...
                            div {
                                ...
                                toggleableSection(
                                    {
                                        text(i18nContext.get(DashboardI18nKeysData.Drops.ShowGuildInformationOnTransactions.Toggle))
                                    },
                                    {
                                        text(i18nContext.get(DashboardI18nKeysData.Drops.ShowGuildInformationOnTransactions.Description))
                                    },
                                    dropsConfig?.get(DropsConfigs.showGuildInformationOnTransactions) ?: false,
                                    "showGuildInformationOnTransactions",
                                    true
                                ) {
                                    fieldWrappers {
                                        fieldWrapper {
                                            ...
                                            textInput {
                                                attributes["bliss-post"] = "/${i18nContext.get(I18nKeysData.Website.LocalePathId)}/guilds/${guild.idLong}/drops/invite"
                                                ...
                                                attributes["loritta-config"] = "true"
                                                name = "inviteId"
                                                val configuredInviteCode = dropsConfig?.get(DropsConfigs.guildInviteCode)
                                                value = if (configuredInviteCode != null && configuredInviteCode.isNotBlank()) "https://discord.gg/$configuredInviteCode" else ""
                                                placeholder = "https://discord.gg/loritta"
                                            }

                                            div {
                                                id = "invite-result"
                                                text(i18nContext.get(DashboardI18nKeysData.Drops.ShowGuildInformationOnTransactions.LoadingInviteInformation))
                                            }
                                        }
                                    }
                                }
