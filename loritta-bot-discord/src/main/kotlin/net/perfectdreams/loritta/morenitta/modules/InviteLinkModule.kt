...
class InviteLinkModule(val loritta: LorittaBot) : MessageReceivedModule {
    companion object {
        val cachedInviteLinks =
            Caffeine.newBuilder().expireAfterAccess(30L, TimeUnit.MINUTES).build<Long, List<String>>().asMap()
    }
    ...
    override suspend fun matches(
        ...
        val inviteBlockerConfig = serverConfig.getCachedOrRetreiveFromDatabase<InviteBlockerConfig?>(
            loritta, ServerConfig::inviteBlockerConfig
        ) ?: return false
    ...
    override suspend fun handle(
        ...
        for (matcher in validMatchers) {
            ...
            for (url in urls) {
                ...
                if (inviteBlockerConfig.deleteMessage && guild.selfMember.hasPermission(
                        message.guildChannel, Permission.MESSAGE_MANAGE
                    )
                ...
                if (inviteBlockerConfig.tellUser && !warnMessage.isNullOrEmpty() && message.guildChannel.canTalk()) {
                    if (event.member != null && event.member.hasPermission(Permission.MANAGE_SERVER)) {
                        ...
                        val topRole =
                            event.member.roles.sortedByDescending { it.position }.firstOrNull { !it.isPublicRole }

                        if (topRole != null) {
                            val button = loritta.interactivityManager.buttonForUser(message.author,
                                ButtonStyle.PRIMARY,
                                ...
                                val success = loritta.pudding.transaction {
                                    ...
                                    if (net.perfectdreams.loritta.cinnamon.pudding.tables.servers.ServerRolePermissions.select {
                                            net.perfectdreams.loritta.cinnamon.pudding.tables.servers.ServerRolePermissions.guild eq guild.idLong and (net.perfectdreams.loritta.cinnamon.pudding.tables.servers.ServerRolePermissions.roleId eq topRole.idLong) and (net.perfectdreams.loritta.cinnamon.pudding.tables.servers.ServerRolePermissions.permission eq net.perfectdreams.loritta.common.utils.LorittaPermission.ALLOW_INVITES)
                                        }.count() == 1L) {
                                        return@transaction false
                                    }

                                    net.perfectdreams.loritta.cinnamon.pudding.tables.servers.ServerRolePermissions.insert {
                                        it[net.perfectdreams.loritta.cinnamon.pudding.tables.servers.ServerRolePermissions.guild] =
                                            guild.idLong
                                        it[net.perfectdreams.loritta.cinnamon.pudding.tables.servers.ServerRolePermissions.roleId] =
                                            topRole.idLong
                                        it[net.perfectdreams.loritta.cinnamon.pudding.tables.servers.ServerRolePermissions.permission] =
                                            net.perfectdreams.loritta.common.utils.LorittaPermission.ALLOW_INVITES
                                    }
                                    return@transaction true
                                }

                                // Update message updates the original interaction message, in this case, where the button is
                                deferredEdit.editOriginalComponents(ActionRow.of(context.event.component.asDisabled()))
                                    .await()
                                    ...
                                    if (success) {
                                        ...
                                        styled(
                                            context.i18nContext.get(
                                                I18nKeysData.Modules.InviteBlocker.RoleAlreadyHasInviteBlockerBypass(
                                                    "<@&${topRole.idLong}>"
                                                )
                            ...
                            message.guildChannel.sendMessageAsync(MessageCreate {
                                mentions {}
