...
class CowboyProfileCreator(loritta: LorittaBot) : StaticProfileCreator(loritta, "cowboy") {
	override suspend fun create(
		...
		val avatar = (LorittaUtils.downloadImage(loritta, user.avatarUrl) ?: Constants.DEFAULT_DISCORD_BLUE_AVATAR).getScaledInstance(147, 147, BufferedImage.SCALE_SMOOTH)
		...
		ProfileUtils.getMarriageInfo(loritta, userProfile)?.let { (marriage, marriedWith) ->
			...
			graphics.font = latoBold16
			ImageUtils.drawCenteredString(graphics, DateUtils.formatDateDiff(i18nContext, marriage.marriedSince.toEpochMilliseconds(), System.currentTimeMillis(), 3), Rectangle(311, 0 + 18 + 24, 216, 14), latoBold16)
		}
