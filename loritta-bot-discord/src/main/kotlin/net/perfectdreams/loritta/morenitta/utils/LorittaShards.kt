...
class LorittaShards(val loritta: LorittaBot, val shardManager: ShardManager) {
    ...
    // Bulk retrieve variant
    suspend fun retrieveUsersInfoById(ids: Set<Long>): Map<Long, CachedUserInfo?> {
        if (ids.isEmpty())
            return emptyMap()

        val usersToInformation = mutableMapOf<Long, CachedUserInfo?>()

        for (id in ids) {
            // Ao dar retrieve na info do user, primeiro iremos tentar verificar se a gente tem ele no user cache do JDA
            val userInJdaCache = loritta.lorittaShards.getUserById(id)
            if (userInJdaCache != null)
                usersToInformation[id] = transformUserToCachedUserInfo(userInJdaCache)

            // Se não tiver, vamos verificar no cache local de retrieved users
            val cachedRetrievedUser = cachedRetrievedUsers.getIfPresent(id)
            if (cachedRetrievedUser != null)
                usersToInformation[id] = transformUserToCachedUserInfo(cachedRetrievedUser.get())
        }

        val haventFoundYetIdsQueryDatabase = ids.filter { it !in usersToInformation }

        // Already found all of them, yay :3
        if (haventFoundYetIdsQueryDatabase.isEmpty())
            return usersToInformation

        // Oh no... we haven't found everyone yet!
        // Because we haven't, we'll now verify on
        // Sa database externa
        val cachedUsers = loritta.newSuspendedTransaction {
            CachedDiscordUsers.selectAll()
                .where { CachedDiscordUsers.id inList haventFoundYetIdsQueryDatabase }
                .toList()
        }

        for (cachedUser in cachedUsers) {
            usersToInformation[cachedUser[CachedDiscordUsers.id].value] = CachedUserInfo(
                cachedUser[CachedDiscordUsers.id].value,
                cachedUser[CachedDiscordUsers.name],
                cachedUser[CachedDiscordUsers.discriminator],
                cachedUser[CachedDiscordUsers.globalName],
        ...
        val haventFoundYetIdsQueryDiscord = ids.filter { it !in usersToInformation }

        // Already found all of them, yay :3
        if (haventFoundYetIdsQueryDiscord.isEmpty())
            return usersToInformation

        for (id in haventFoundYetIdsQueryDiscord) {
            // E se *ainda* não tiver, iremos dar retrieve
            val discordUser = retrieveUserById(id)

            usersToInformation[id] = if (discordUser != null) {
                transformUserToCachedUserInfo(discordUser)
            } else null
        }
