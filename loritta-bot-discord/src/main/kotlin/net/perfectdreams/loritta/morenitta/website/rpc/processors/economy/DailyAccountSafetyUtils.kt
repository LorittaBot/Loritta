...
object DailyAccountSafetyUtils {
    ...
    suspend fun checkIfUserCanPayout(loritta: LorittaBot, userIdentification: DiscordOAuth2UserIdentification, ip: String): AccountDailyPayoutCheckResult {
        ...
        val lastReceivedDailyAt = loritta.newSuspendedTransaction {
            net.perfectdreams.loritta.cinnamon.pudding.tables.Dailies.selectAll().where { Dailies.receivedById eq userIdentification.id and (Dailies.receivedAt greaterEq todayAtMidnight) }.orderBy(
        ...
        if (lastReceivedDailyAt.isNotEmpty()) {
            logger.info { "Blocking ${userIdentification.id} because they already received the daily reward today" }
            if (!loritta.isOwner(userIdentification.id)) {
        ...
        if (isIPv6) {
            ...
            for (dailyIp in allDailiesThatWereReceivedToday) {
                ...
                if (dailyIpHouseholdPrefixBlocks == selfIpHouseholdPrefixBlocks) {
                    ...
                    if (expandedDailyIp == expandedSelfIp) {
                        logger.info { "Blocking ${userIdentification.id} because they already received the daily reward today (Same IPv6)" }
                        if (!loritta.isOwner(userIdentification.id)) {
                    ...
                    logger.info { "Tripped ${userIdentification.id} same household prefix checks (IPv6 - Trip count: $trippedSamePrefixChecks - Household prefix is $dailyIpHouseholdPrefixBlocks)" }

                    if (trippedSamePrefixChecks == 3) {
                        logger.info { "Blocking ${userIdentification.id} because they already received the daily reward today (IPv6 - Too many rewards on the same IPv6 prefix - Household prefix is $dailyIpHouseholdPrefixBlocks)" }
                        if (!loritta.isOwner(userIdentification.id)) {
                            return AccountDailyPayoutCheckResult.AlreadyGotTheDailyRewardSameIp(
                                tomorrowAtMidnight,
                                ip
            ...
            if (lastReceivedDailyAt.isNotEmpty() || sameIpDailyOneHourAgoAt.isNotEmpty()) {
                logger.info { "Blocking ${userIdentification.id} because they already received the daily reward today (Same IPv4)" }
                if (!loritta.isOwner(userIdentification.id)) {
                    return AccountDailyPayoutCheckResult.AlreadyGotTheDailyRewardSameAccount(tomorrowAtMidnight)
                }
            }
            ...
            if (sameIpDailyAt.isNotEmpty()) {
                ...
                if (sameIpDailyAt.size >= 3) {
                    logger.info { "Blocking ${userIdentification.id} because they already received the daily reward today (IPv4 - Too many rewards on the same IP)" }
                    return AccountDailyPayoutCheckResult.AlreadyGotTheDailyRewardSameIp(
                        tomorrowAtMidnight,
                        ip
                    )
    ...
    suspend fun verifyIfAccountAndIpAreSafe(loritta: LorittaBot, userIdentification: DiscordOAuth2UserIdentification, ip: String): AccountCheckResult {
