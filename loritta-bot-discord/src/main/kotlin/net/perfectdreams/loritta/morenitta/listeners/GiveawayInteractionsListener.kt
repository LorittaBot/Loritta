            ...
            GlobalScope.launch {
                ...
                m.giveawayManager.lockGiveaway(dbId, false) {
                    val state = m.transaction {
                        ...
                        val allowedRoles =
                            giveaway[Giveaways.allowedRoles]?.let { Json.decodeFromString<GiveawayRoles>(it) }
                    ...
                    when (state) {
                                ...
                                MessageEdit {
                                    ...
                                    actionRow(
                                        m.interactivityManager.buttonForUser(
                                            event.user,
                                            ButtonStyle.DANGER,
                                            i18nContext.get(GiveawayManager.I18N_PREFIX.JoinGiveaway.LeaveGiveaway),
                                            { loriEmoji = Emotes.LoriBear }) {
                            ...
                            if (state.allowedRoles.isAndCondition) {
                                deferredReply.editOriginal(
                                    MessageEdit {
                                        styled(
                                            i18nContext.get(
                                                GiveawayManager.I18N_PREFIX.JoinGiveaway.MissingRolesAnd(
                                                    state.allowedRoles.roleIds.joinToString { "<@&${it}>" })
                                            ),
                                            Emotes.LoriSob
                                        )
                                    })
                                ...
                                deferredReply.editOriginal(
                                    MessageEdit {
                                        styled(
                                            i18nContext.get(
                                                GiveawayManager.I18N_PREFIX.JoinGiveaway.MissingRolesOr(
                                                    state.allowedRoles.roleIds.joinToString { "<@&${it}>" })
                                            ),
                                            Emotes.LoriSob
                                        )
                            ...
                            if (state.deniedRoles.isAndCondition) {
                                deferredReply.editOriginal(
                                    MessageEdit {
                                        styled(
                                            i18nContext.get(
                                                GiveawayManager.I18N_PREFIX.JoinGiveaway.BlockedRolesAnd(
                                                    state.deniedRoles.roleIds.joinToString { "<@&${it}>" })
                                            ),
                                            Emotes.LoriSob
                                        )
                                    })
                                ...
                                deferredReply.editOriginal(
                                    MessageEdit {
                                        styled(
                                            i18nContext.get(
                                                GiveawayManager.I18N_PREFIX.JoinGiveaway.BlockedRolesOr(
                                                    state.deniedRoles.roleIds.joinToString { "<@&${it}>" })
                                            ),
                                            Emotes.LoriSob
                                        )
                        ...
                        is GiveawayState.NeedsToWinMoreEmojiFightBets -> {
                            deferredReply.editOriginal(
                                MessageEdit {
                                    styled(
                                        i18nContext.get(
                                            GiveawayManager.I18N_PREFIX.JoinGiveaway.YouNeedToWinMoreEmojiFightBets(
                                                state.matchesRequired,
                                                state.matchesAlreadyWon
                                            )
                                        ),
                                        Emotes.LoriSob
                                    )
                        ...
                        is GiveawayState.NeedsToLoseMoreEmojiFightBets -> {
                            deferredReply.editOriginal(
                                MessageEdit {
                                    styled(
                                        i18nContext.get(
                                            GiveawayManager.I18N_PREFIX.JoinGiveaway.YouNeedToLoseMoreEmojiFightBets(
                                                state.matchesRequired,
                                                state.matchesAlreadyLost
                                            )
                                        ),
                                        Emotes.LoriSob
                                    )
                        ...
                        is GiveawayState.Success -> {
                            ...
                            (m.giveawayManager.giveawayMessageUpdateMutexes[dbId] ?: error("Missing message update mutex for giveaway $dbId!")).withLock {
                                m.giveawayManager.giveawayMessageUpdateJobs[dbId]?.cancel()
                                m.giveawayManager.giveawayMessageUpdateJobs[dbId] =
                                    GlobalScope.launch(m.coroutineDispatcher) {
                                        ...
                                        val channel =
                                            event.jda.getChannel<MessageChannel>(giveaway[Giveaways.textChannelId])
