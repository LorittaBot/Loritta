        ...
        suspend fun createRankMessage(
            ...
            val (totalCount, profiles) = loritta.pudding.transaction {
                if (guild != null) {
                    ...
                    val profilesInTheQuery = Profiles.innerJoin(GuildProfiles, { id }, { userId })
                        .selectAll().where {
                    ...
                    val profiles = Profiles
                        .selectAll().where { Profiles.id notInSubQuery UsersService.validBannedUsersList(System.currentTimeMillis()) and (Profiles.id notInSubQuery UsersService.botTokenUsersList()) }
                        .orderBy(Profiles.money, SortOrder.DESC)
                        .limit(5)
                        .offset(page * 5)
                        .toList()
