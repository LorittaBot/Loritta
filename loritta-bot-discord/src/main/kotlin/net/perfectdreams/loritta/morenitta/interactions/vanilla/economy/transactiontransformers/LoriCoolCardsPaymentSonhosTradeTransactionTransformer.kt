    ...
    override suspend fun transform(
        ...
        val receiverUserInfo =
            cachedUserInfos.getOrPut(transaction.receivedBy) {
                HarmonyLoggerFactory.logger {}.value.info { "LoriCoolCardsPaymentSonhosTradeTransactionTransformer#retrieveUserInfoById - UserId: ${transaction.receivedBy}" }

                loritta.lorittaShards.retrieveUserInfoById(transaction.receivedBy)
            }
        val giverUserInfo =
            cachedUserInfos.getOrPut(transaction.givenBy) {
                HarmonyLoggerFactory.logger {}.value.info { "LoriCoolCardsPaymentSonhosTradeTransactionTransformer#retrieveUserInfoById - UserId: ${transaction.givenBy}" }
