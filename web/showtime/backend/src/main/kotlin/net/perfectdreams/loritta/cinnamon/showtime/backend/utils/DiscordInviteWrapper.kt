...
object DiscordInviteWrapper {
    ...
    fun FlowContent.sparklyPowerServerInvite(showtimeBackend: ShowtimeBackend, i18nContext: I18nContext) = discordInvite(
        i18nContext,
        "${showtimeBackend.etherealGambiClient.baseUrl}/sparklypower/sparklypower-server-icon-64.gif",
        "SparklyPower \uD83D\uDC8E\uD83C\uDFAE",
        {
            imgSrcSetFromEtherealGambi(
                showtimeBackend,
                // TODO: Fix this SUPER HACK:tm: - (However do we really care about this? While it does suspend, it should be 99,99% of the times in memory, right)
                runBlocking { showtimeBackend.getOrRetrieveImageInfo("sparklypower/emotes/pantufa-hangloose")!! },
                "png",
                "1.5em"
            ) {
                classes = setOf("inline-emoji")
