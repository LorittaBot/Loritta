...
import net.perfectdreams.loritta.cinnamon.showtime.backend.utils.imgSrcSet
...
class DonateView(
                                            ...
                                            if (needsToLogin) {
                                                a(href = url) {
                                                    div(classes = "button-discord button-discord-info pure-button") {
                                                        ...
                                                        showtimeBackend.svgIconManager.gift.apply(this)
    ...
    inner class DonationReward(val name: String, val minimumDonation: Double, val doNotDisplayInPlans: Boolean, val callback: TD.(Double) -> Unit = { column ->
        if (column >= minimumDonation) {
            showtimeBackend.svgIconManager.check.apply(this)
        } else {
            showtimeBackend.svgIconManager.xmark.apply(this)
        }
    })
}