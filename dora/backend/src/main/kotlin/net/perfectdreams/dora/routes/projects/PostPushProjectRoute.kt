...
class PostPushProjectRoute(val dora: DoraBackend) : RequiresProjectAuthDashboardRoute(dora, "/push") {
    ...
    val projectSlug = call.parameters.getOrFail("projectSlug")
        ...
        val (project, languageTargets) = dora.pudding.transaction {
            ...
            Pair(project, languageTargets)
        }

        for (languageTarget in languageTargets) {
            dora.pushTranslations(project, languageTarget)
        }
