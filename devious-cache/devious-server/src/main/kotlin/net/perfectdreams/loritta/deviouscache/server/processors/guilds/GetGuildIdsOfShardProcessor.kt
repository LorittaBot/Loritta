...
class GetGuildIdsOfShardProcessor(val m: DeviousCache) {
    ...
    suspend fun process(request: GetGuildIdsOfShardRequest): DeviousResponse {
        ...
        val guildsOnThisShard = m.guilds.keys.filter { (it.value.toLong() shr 22).rem(request.maxShards).toInt() == request.shardId }

        return GetGuildIdsOfShardResponse(guildsOnThisShard)
    }
}