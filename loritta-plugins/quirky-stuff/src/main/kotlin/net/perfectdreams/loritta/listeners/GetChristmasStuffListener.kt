...
class GetChristmasStuffListener(val config: QuirkyConfig) : ListenerAdapter() {
	companion object {
	...
	override fun onGuildMessageReactionAdd(event: GuildMessageReactionAddEvent) {
		...
		GlobalScope.launch(loritta.coroutineDispatcher) {
			...
			if (whatKind == null || event.reactionEmote.id != (whatKind.emote.split(":").last()))
				return@launch
			...
			if (System.currentTimeMillis() - 900_000 >= message.timeCreated.toInstant().toEpochMilli()) {
				try {
					event.user.openPrivateChannel().await().sendMessage("O paranauê que você coletou está velho demais!")
							.await()
				} catch (e: Exception) {}
				return@launch
			}
