    ...
    override fun run() {
        while (true) {
            try {
                ...
                connection.use {
                    val selectStatement = it.prepareStatement("""SELECT id, "type", payload FROM ${DiscordGatewayEvents.tableName} WHERE shard % ${m.config.replicas} = ${m.replicaId} ORDER BY id FOR UPDATE SKIP LOCKED LIMIT ${m.config.eventsPerBatch};""")
