...
class GatewayProxy(
    ...
    fun start(): MutableMap<Int, ProxiedKordGateway> {
                ...
                when (event) {
                    is Frame.Text -> {
                        try {
                            ...
                            val gatewayProxyEvent = Json.decodeFromString<GatewayProxyEvent>(event.data.toString(Charsets.UTF_8))
                            val kordEvent = KordDiscordEventUtils.parseEventFromJsonObject(gatewayProxyEvent.event)

                            if (kordEvent != null) {
                                // Emit the event to our proxied instances
                                val proxiedKordGateway = proxiedKordGateways[gatewayProxyEvent.shardId] ?: error("Received event for shard ID ${gatewayProxyEvent.shardId}, but we don't have a ProxiedKordGateway instance associated with it!")
                                coroutineScope.launch {
                                    proxiedKordGateway.events.emit(kordEvent)
                                }
                            }
                        } catch (e: Exception) {
