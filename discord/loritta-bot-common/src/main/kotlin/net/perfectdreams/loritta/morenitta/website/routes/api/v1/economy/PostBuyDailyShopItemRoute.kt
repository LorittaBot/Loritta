...
class PostBuyDailyShopItemRoute(loritta: LorittaBot) :
    RequiresAPIDiscordLoginRoute(loritta, "/api/v1/economy/daily-shop/buy/{type}/{internalName}") {
    companion object {
            ...
            loritta.newSuspendedTransaction {
                if (type == "background") {
                    ...
                    for (creatorId in createdBy) {
                        ...
                        val discordId =
                            author.socialNetworks?.filterIsInstance<FanArtArtist.SocialNetwork.DiscordSocialNetwork>()
                                ?.firstOrNull()?.id
                                ?: continue
