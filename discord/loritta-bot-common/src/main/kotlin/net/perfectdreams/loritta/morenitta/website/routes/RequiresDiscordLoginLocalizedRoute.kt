...
abstract class RequiresDiscordLoginLocalizedRoute(loritta: LorittaBot, path: String) : LocalizedRoute(loritta, path) {
            ...
            if (code == null) {
                if (discordAuth == null) {
                    ...
                    if (call.request.header("User-Agent") == Constants.DISCORD_CRAWLER_USER_AGENT) {
                        ...
                        redirect(
                            loritta.config.loritta.discord.authorizationUrl + "&state=${
                                Base64.getEncoder().encodeToString(state.toString().toByteArray()).encodeToUrl()
                            }", false
                        )
                    }
                }
                ...
                val userIdentification = if (code == "from_master") {
                    ...
                    call.sessions.set(
                        session.copy(
                            ...
                            storedDiscordAuthTokens = auth.toJsonBase64()
                        )
                    )
                ...
                if (guildId != null) {
                    if (code != "from_master") {
                        ...
                        if (cluster.getUrl(loritta) != hostHeader) {
                            ...
                            // Vamos redirecionar!
                            redirect(
                                "$scheme://${cluster.getUrl(loritta)}/dashboard?guild_id=${guildId}&code=from_master",
                                true
                            )
                        }
                    }
                        ...
                        if (guild != null) {
                            ...
                            if (user != null) {
                                ...
                                if (member != null) {
                                    ...
                                    if (!user.isBot && (member.hasPermission(Permission.ManageGuild) || member.hasPermission(
                                        ...
                                        if (blacklisted != null) {
                                            ...
                                            val message =
                                                locale.getList("website.router.blacklistedServer", blacklistedReason)

                                            runCatching {
                                                user.openPrivateChannel().sendMessage(message.joinToString("\n"))
                                            }
                                        ...
                                        if (bannedState != null) { // Dono blacklisted
                                            ...
                                            val message = locale.getList(
                                                "website.router.ownerLorittaBanned",
                                                "<@$guildOwnerId>",
                ...
                redirect(
                    "$scheme://$hostHeader/dashboard",
                    false
                ) // Redirecionar para a dashboard, mesmo que nós já estejamos lá... (remove o "code" da URL)
            }
        }
    ...
    open suspend fun onUnauthenticatedRequest(call: ApplicationCall, locale: BaseLocale) {
        ...
        val state = JsonObject()
        state["redirectUrl"] = LorittaWebsite.WEBSITE_URL.substring(
            0,
            LorittaWebsite.Companion.WEBSITE_URL.length - 1
        ) + call.request.path()
        redirect(
            loritta.config.loritta.discord.authorizationUrl + "&state=${
                Base64.getEncoder().encodeToString(state.toString().toByteArray()).encodeToUrl()
