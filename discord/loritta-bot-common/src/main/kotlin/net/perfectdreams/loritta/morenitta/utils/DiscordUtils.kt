...
object DiscordUtils {
     ...
     * @return the shard ID
     */
    fun getLorittaClusterForGuildId(
        loritta: LorittaBot,
        id: Long
    ): LorittaConfig.LorittaClustersConfig.LorittaClusterConfig {
        val shardId = getShardIdFromGuildId(loritta, id)
        return getLorittaClusterForShardId(loritta, shardId)
    }

    fun isCurrentClusterHandlingGuildId(loritta: LorittaBot, id: Long) =
        getLorittaClusterForGuildId(loritta, id).id == loritta.lorittaCluster.id
     ...
     * @return the shard ID
     */
    fun getShardIdFromGuildId(loritta: LorittaBot, id: Long) =
        getShardIdFromGuildId(id, loritta.config.loritta.discord.maxShards)
     ...
     * @return the cluster
     */
    fun getLorittaClusterForShardId(
        loritta: LorittaBot,
        id: Int
    ): LorittaConfig.LorittaClustersConfig.LorittaClusterConfig {
        val lorittaShard = loritta.config.loritta.clusters.instances.firstOrNull { id in it.minShard..it.maxShard }
        return lorittaShard
            ?: throw RuntimeException("Frick! I don't know what is the Loritta Shard for Discord Shard ID $id")
    }
    ...
    suspend fun extractUserFromString(
        ...
        if (usersInContext != null && extractUserViaMention) {
            for (user in usersInContext) {
                ...
                ) { // O replace é necessário já que usuários com nick tem ! no mention (?)
        ...
        if (guild != null) {
            if (extractUserViaNameAndDiscriminator) {
                ...
                if (split.size == 2) {
                    ...
                    } // We don't really care if it is in a invalid format
                }
            }
