...
class NyanCatCommand(loritta: LorittaBot) :
    AbstractCommand(loritta, "nyan", category = net.perfectdreams.loritta.common.commands.CommandCategory.IMAGES) {
    companion object {
...
// TODO: Fix Usage
    ...
    override suspend fun run(context: CommandContext, locale: BaseLocale) {
        ...
        val bi = BufferedImage(
            catLeft.getWidth(null) + catRight.getWidth(null) + catMiddle.getWidth(null) * times,
            catLeft.getHeight(null),
        ...
        while (times > idx) {
            ...
            for (i in 0..randomDots) {
                ...
                if (invalidDotsPositionsX.contains(randomX) || invalidDotsPositionsX.contains(randomX - 1) || invalidDotsPositionsX.contains(
                        randomX + 1
                    )
                ) {
                    continue
                ...
                if (invalidDotsPositionsY.contains(randomY) || invalidDotsPositionsY.contains(randomY - 1) || invalidDotsPositionsY.contains(
                        randomY + 1
                    )
                ) {
                    continue
        ...
        context.sendFile(
            ...
            ), "nyan_cat.png", context.getAsMention(true)
        )
    }
}