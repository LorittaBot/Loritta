...
class PostPerfectPaymentsCallbackRoute(val loritta: LorittaBot) : BaseRoute("/api/v1/callbacks/perfect-payments") {
    companion object {
        ...
        suspend fun sendPaymentApprovedDirectMessage(
            loritta: LorittaBot,
            userId: Long,
            locale: BaseLocale,
            supportUrl: String
        ...
        if (status == "CHARGED_BACK") {
                ...
                if (internalPayment.reason == PaymentReason.SONHOS_BUNDLE) {
                    ...
                    if (bundleType == "dreams") {
                        ...
                        if (bundle != null) {
                            ...
                            chargebackedQuantity[internalPayment.userId] =
                                chargebackedQuantity.getOrPut(internalPayment.userId) { AtomicLong() }
            ...
            loritta.newSuspendedTransaction {
                BannedUsers.insert {
                    ...
                    it[expiresAt] = null
                    it[reason] =
                        "Chargeback/Requesting your money back after a purchase! Why do you pay for something and then chargeback your payment even though you received your product? Payment ID: $referenceId; Gateway: $gateway"
                }
            }
            ...
            if (metadata != null) {
                ...
                if (bundleType == "dreams") {
                    ...
                    val profile =
                        loritta.getLorittaProfileAsync(internalPayment.userId) ?: loritta.getOrCreateLorittaProfile(
                            internalPayment.userId
                        )
                    ...
                    sendPaymentApprovedDirectMessage(
                        loritta,
                        internalPayment.userId,
            ...
            loritta.newSuspendedTransaction {
                ...
                if (internalPayment.reason == PaymentReason.DONATION) {
                    if (isKeyRenewal) {
                        ...
                        internalPayment.expiresAt =
                            donationKey.expiresAt // Fixes bug where key renewals breaks user features due to the difference in the expiration date
                    } else {
