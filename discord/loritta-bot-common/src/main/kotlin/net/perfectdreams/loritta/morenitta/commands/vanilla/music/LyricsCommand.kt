package net.perfectdreams.loritta.morenitta.commands.vanilla.music
...
class LyricsCommand(loritta: LorittaBot) : AbstractCommand(
    loritta,
    "lyrics",
    listOf("letra", "letras"),
    ...
    override suspend fun run(context: CommandContext, locale: BaseLocale) {
        ...
        if (split.size == 2) {
            ...
            val songInfo = retrieveSongInfoFromLyricWikia(artist, musicName)
                ...
                ?: retrieveSongInfoFromVagalume(
                    artist,
                    musicName
                ) // Pesquisa no Vagalume, isto deve ser a última opção!
            ...
            val useHighResolution =
                3 > columns.size // Para evitar OutOfMemoryExceptions, vamos fazer fallback de resolução
            ...
            val c =
                Canvas() // Canvas funciona até em headless mode, e é um jeito para a gente conseguir pegar as font metrics da fonte!
            ...
            for (column in columns) {
                val biggestString =
                    column.maxByOrNull { getStringWidth(it, lyricFont, lyricFontMetrics, fallbackFontMetrics) }!!
    ...
    fun getStringWidth(
        str: String,
        lyricFont: Font,
        lyricFontMetrics: FontMetrics,
        fallbackFontMetrics: FontMetrics
    ): Int {
        var maxSize = 0
        for (ch in str) {
    ...
    fun retrieveSongInfoFromLyricWikia(artist: String, musicName: String): SongInfo? {
        ...
        val response = Jsoup.connect(
            "http://lyrics.wikia.com/wiki/${
                artist.replace(" ", "_").encodeToUrl()
            }:${musicName.replace(" ", "_").encodeToUrl()}"
        )
    ...
    fun retrieveSongInfoFromGenius(artist: String, musicName: String): SongInfo? {
        val response = Jsoup.connect(
            "https://genius.com/${artist.replace(" ", "-").encodeToUrl()}-${
                musicName.replace(" ", "-").encodeToUrl()
            }-lyrics/"
        )
            .ignoreHttpErrors(true)
            .execute()
        ...
        val artistName =
            document.getElementsByClass("header_with_cover_art-primary_info-primary_artist").firstOrNull()?.text()
        ...
        val prettyPrintedBodyFragment =
            Jsoup.clean(lyrics, "", Safelist.none().addTags("br", "p"), Document.OutputSettings().prettyPrint(true))
        return SongInfo(
            artistName ?: artist,
            songName ?: musicName,
            coverArtUrl,
    ...
    fun retrieveSongInfoFromSongLyrics(artist: String, musicName: String): SongInfo? {
        val response = Jsoup.connect(
            "http://www.songlyrics.com/${artist.replace(" ", "-").encodeToUrl()}/${
                musicName.replace(
                    " ",
                    "-"
                ).encodeToUrl()
    ...
    fun retrieveSongInfoFromLetrasMus(artist: String, musicName: String): SongInfo? {
        val response = try {
            Jsoup.connect(
                "https://www.letras.mus.br/${artist.replace(" ", "-").encodeToUrl()}/${
                    musicName.replace(
                        " ",
                        "-"
                    ).encodeToUrl()
    ...
    fun retrieveSongInfoFromVagalume(artist: String, musicName: String): SongInfo? {
        val request =
            HttpRequest.get("https://api.vagalume.com.br/search.artmus?apikey=lolidkthekey&q=${artist.encodeToUrl()} ${musicName.encodeToUrl()}&limit=1")
                .body()
