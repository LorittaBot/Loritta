...
class RollCommand(loritta: LorittaBot) : AbstractCommand(
    loritta,
    "roll",
    listOf("rolar", "dice", "dado"),
    ...
    override suspend fun run(context: CommandContext, locale: BaseLocale) {
        ...
        if (context.args.isNotEmpty()) {
            ...
            try {
                ...
                if (context.args.size >= 2) {
                    ...
                    try {
                        MathUtils.evaluate(
                            LorittaBot.RANDOM.nextLong(lowerBound, upperBound + 1).toString() + expression
                        ).toInt().toString()
        ...
        val message =
            context.locale["$LOCALE_PREFIX.result", context.getAsMention(false), upperBound.toString(), finalResult.toInt()]
