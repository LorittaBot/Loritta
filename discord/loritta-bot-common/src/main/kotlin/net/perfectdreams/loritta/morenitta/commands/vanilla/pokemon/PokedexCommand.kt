package net.perfectdreams.loritta.morenitta.commands.vanilla.pokemon
...
class PokedexCommand(loritta: LorittaBot) : AbstractCommand(
    loritta,
    "pokedex",
    listOf("pokédex"),
    net.perfectdreams.loritta.common.commands.CommandCategory.POKEMON
) {
    ...
    override suspend fun run(context: CommandContext, locale: BaseLocale) {
        if (context.args.size == 1) {
            ...
            var http = HttpRequest.get("https://veekun.com/dex/pokemon/${context.args[0].toLowerCase().encodeToUrl()}")
            ...
            embed.apply {
                setTitle(
                    "<:pokeball:412575443024216066> $name",
                    "https://veekun.com/dex/pokemon/${context.args[0].toLowerCase()}"
                )
            ...
            embed.addField(
                locale["commands.command.pokedex.addedInGen"],
                pokeInfoValue[0].getElementsByTag("img")[0].attr("alt"),
                true
            )
            ...
            var strTraining =
                "**${context.locale["commands.command.pokedex.baseExp"]}:** ${trainingInfoValue[0].text()}" +
