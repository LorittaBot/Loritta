...
class GiveawayCommand(loritta: LorittaBot) : DiscordAbstractCommandBase(
    loritta,
    listOf("giveaway", "sorteio"),
        ...
        executesDiscord {
            ...
            for (cmd in commands) {
                val toBeAdded = run {
                    ...
                    "**${context.serverConfig.commandPrefix}${cmd.labels.firstOrNull()}** $usageWithinCodeBlocks» ${
                        cmd.command().description(loritta.localeManager.getLocaleById(context.serverConfig.localeId))
                    }\n"
                }
