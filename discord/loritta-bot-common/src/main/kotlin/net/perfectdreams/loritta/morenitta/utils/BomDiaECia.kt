...
class BomDiaECia(val loritta: LorittaBot) {
    ...
    fun announceWinner(channel: Channel, guild: Guild, user: User) {
        ...
        loritta.legacyLocales.forEach { localeId, locale ->
            val message = MessageBuilder().append(
                "<:yudi:446394608256024597> **|** Parabéns `${
                    user.name.stripCodeMarks().stripLinks()
                }#${user.discriminator}` (`${user.id}`) por ter ligado primeiro em `${
                    guild.name.stripCodeMarks().stripLinks()
                }` (`${guild.id}`)!"
            )
        ...
        GlobalScope.launch(loritta.coroutineDispatcher) {
            ...
            if (triedToCall.isNotEmpty()) {
                ...
                runCatching {
                    val message =
                        channel.sendMessage("<:yudi:446394608256024597> **|** Sabia que ${user.asMention} foi $pronoun primeir$pronoun de **${triedToCall.size} usuários** a conseguir ligar no Bom Dia & Cia? ${Emotes.LORI_OWO}")
    ...
    fun getActiveTextChannels(): Set<Channel> {
        ...
        runBlocking {
            activeTextChannels.entries.forEach {
                ...
                if (textChannel != null && textChannel.canTalk() && textChannel.guild.retrieveSelfMember()
                        .hasPermission(
