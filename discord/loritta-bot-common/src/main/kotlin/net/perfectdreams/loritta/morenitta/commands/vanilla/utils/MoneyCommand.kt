...
class MoneyCommand(loritta: LorittaBot) : AbstractCommand(
    loritta,
    "money",
    listOf("dinheiro", "grana"),
        ...
        if (context.args.size >= 2) {
            ...
            if (from == to) { // :rolling_eyes:
                ...
                val euroValueInCurrency = exchangeRates[from] ?: run {
                    context.reply(
                        LorittaReply(
                            message = locale["commands.command.money.invalidCurrency"].msgFormat(
                                from,
                                exchangeRates.keys.joinToString(transform = { "`$it`" })
                            ),
