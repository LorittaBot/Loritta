...
class MSNProfileCreator(loritta: LorittaBot) : StaticProfileCreator(loritta, "msn") {
    override suspend fun create(
        ...
        val senderAvatar = (LorittaUtils.downloadImage(loritta, imageToBeDownload)
            ?: Constants.DEFAULT_DISCORD_BLUE_AVATAR).getScaledInstance(141, 141, BufferedImage.SCALE_SMOOTH)
        ...
        ImageUtils.drawTextWrapSpaces(
            loritta,
            "Não inclua informações como senhas ou número de cartões de crédito em uma mensagem instantânea",
            297,
            224,
            768,
            1000,
        ...
        ImageUtils.drawTextWrapSpaces(
            loritta, /* "Olá, meu nome é ${user.name}! Atualmente eu tenho ${userProfile.dreams} Sonhos, já recebi ${userProfile.receivedReputations.size} reputações, estou em #$position (${userProfile.xp} XP) no rank global e estou em #${localPosition ?: "???"} (${xpLocal?.xp} XP) no rank do ${guild.name}!\n\n${userProfile.aboutMe}" */
            aboutMe,
            297,
            326,
            768,
