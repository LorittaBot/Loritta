...
object NitroBoostUtils {
    ...
    internal fun createBoostTask(
        loritta: LorittaBot,
        config: LorittaConfig.DonatorsOstentationConfig
        ...
        while (true) {
            ...
            try {
                ...
                for (guildWithBoostFeature in guildsWithBoostFeature) {
                    ...
                    val guild =
                        loritta.lorittaShards.getGuildById(guildWithBoostFeature[ServerConfigs.id].value) ?: continue
                ...
                if (loritta.isMainInstance) {
                    ...
                    loritta.newSuspendedTransaction {
                        for (profile in Profiles.select { Profiles.id inList deserveTheRewardUsers }) {
                            val userPayment =
                                mostPayingUsers.firstOrNull { it[Payments.userId] == profile[Profiles.id].value }

                            if (userPayment != null) {
                                val roundedUpPayment = Math.ceil(userPayment[moneySumId]!!.toDouble())
                                val howMuchShouldBeGiven =
                                    ((roundedUpPayment / 20.00) * 2) // Se doou 19.99, será (19.99 / 19.99) * 2 = 2 sonhos por segundo, se foi 39.99, será 4 sonhos, etc
