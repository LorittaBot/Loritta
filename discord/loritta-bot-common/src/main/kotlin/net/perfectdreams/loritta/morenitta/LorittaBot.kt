...
class LorittaBot(
    ...
    companion object {
        ...
        @JvmStatic
        val RANDOM =
            SplittableRandom() // Um splittable RANDOM global, para não precisar ficar criando vários (menos GC)
    ...
    val webhookExecutor = Executors.newScheduledThreadPool(
        Runtime.getRuntime().availableProcessors(),
        ThreadFactoryBuilder().setNameFormat("Webhook Sender %d").build()
    )
    val webhookOkHttpClient = OkHttpClient()

    val legacyCommandManager = CommandManager(this) // Nosso command manager
    var messageInteractionCache = Caffeine.newBuilder().maximumSize(1000L).expireAfterAccess(3L, TimeUnit.MINUTES)
        .build<Long, MessageInteractionFunctions>().asMap()

    var ignoreIds = mutableSetOf<Long>() // IDs para serem ignorados nesta sessão
    val apiCooldown =
        Caffeine.newBuilder().expireAfterAccess(30L, TimeUnit.SECONDS).maximumSize(100).build<String, Long>().asMap()
    ...
    fun createThreadPool(name: String) =
    ...
    suspend fun getUserProfileBackgroundUrl(profile: Profile): String {
        ...
        val activeProfileDesignInternalName =
            this.newSuspendedTransaction { profile.settings.activeProfileDesignInternalName }?.value
        val activeBackgroundInternalName =
            this.newSuspendedTransaction { profile.settings.activeBackgroundInternalName }?.value
        return getUserProfileBackgroundUrl(
            profile.userId,
            settingsId,
            activeProfileDesignInternalName ?: ProfileDesign.DEFAULT_PROFILE_DESIGN_ID,
            activeBackgroundInternalName ?: Background.DEFAULT_BACKGROUND_ID
        )
    }
        ...
        if (background.id == Background.CUSTOM_BACKGROUND_ID) {
            ...
            if (plan.customBackground) {
                ...
                if (resultRow != null) {
                    ...
                    val extension =
                        MediaTypeUtils.convertContentTypeToExtension(resultRow[net.perfectdreams.loritta.morenitta.tables.CustomBackgroundSettings.preferredMediaType])
                    return "${this.config.loritta.dreamStorageService.url}/$dssNamespace/${
                        StoragePaths.CustomBackground(
                            userId,
                            file
                        ).join()
                    }.$extension"
    ...
    fun loadLegacyLocales() {
        ...
        val defaultLocale =
            LorittaBot.GSON.fromJson(localeAsText, LegacyBaseLocale::class.java) // Carregar locale do jeito velho
    ...
    ): T = pudding.transaction(repetitions, transactionIsolation, statement)
     ...
     * @return       the user profile
     */
    suspend fun getLorittaProfileDeferred(userId: Long) =
        GlobalScope.async { pudding.transaction { _getLorittaProfile(userId) } }
    ...
    suspend inline fun <reified T> encodeDataForComponentOnDatabase(
        data: T,
        ttl: Duration = 15.minutes
    ): ComponentOnDatabaseStoreResult<T> {
        ...
        val storedGenericInteractionData = StoredGenericInteractionData(
     ...
     */
    suspend inline fun <reified T> encodeDataForComponentOrStoreInDatabase(
        data: T,
        ttl: Duration = 15.minutes
    ): String {
     ...
     */
    suspend fun sendMessageToUserViaDirectMessage(userId: Snowflake, builder: UserMessageCreateBuilder.() -> (Unit)) =
    ...
    private fun startTasks() {
        ...
        scheduleCoroutineAtFixedRateIfMainReplica(
            1.seconds,
            action = PendingImportantNotificationsProcessor(this@LorittaBot)
        )
