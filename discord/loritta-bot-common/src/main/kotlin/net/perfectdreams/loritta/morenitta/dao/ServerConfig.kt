...
class ServerConfig(id: EntityID<Long>) : Entity<Long>(id) {
     ...
     */
    suspend fun getOrLoadGuildRolesLorittaPermissions(
        loritta: LorittaBot,
        guild: Guild
    ...
    fun getActiveDonationKeysNested() =
        DonationKey.find { DonationKeys.activeIn eq this@ServerConfig.id and (DonationKeys.expiresAt greaterEq System.currentTimeMillis()) }
            .toList()
    ...
    fun getUserDataIfExistsNested(id: Long) =
        GuildProfile.find { (GuildProfiles.guildId eq guildId) and (GuildProfiles.userId eq id) }.firstOrNull()
     ...
     */
    suspend fun <T> getCachedOrRetreiveFromDatabase(
        loritta: LorittaBot,
        property: KMutableProperty1<ServerConfig, *>
    ): T {
     ...
     * Gets or retrieves from the database the object you've requested
     */
    suspend fun <T> getCachedOrRetreiveFromDatabaseAsync(
        loritta: LorittaBot,
        property: KMutableProperty1<ServerConfig, *>
    ): T {
        if (!cachedData.containsKey(property)) {
     ...
     * Gets or retrieves from the database the object you've requested
     */
    suspend fun <T> getCachedOrRetreiveFromDatabaseDeferred(
        loritta: LorittaBot,
        property: KMutableProperty1<ServerConfig, *>
    ): Deferred<T> {
        if (!cachedData.containsKey(property)) {
