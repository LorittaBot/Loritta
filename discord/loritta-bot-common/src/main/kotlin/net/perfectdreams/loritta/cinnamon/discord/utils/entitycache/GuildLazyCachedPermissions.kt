...
class GuildLazyCachedPermissions(
    ...
    override suspend fun retrievePermissions(): Permissions {
        var permissionsResult = cacheService.getPermissions(guildId, userId)
        ...
        if (permissionsResult.missingRoles) {
            logger.warn { "Cache inconsistency detected in $guildId for $userId! ($permissionsResult) We will pull the updated data from Discord's REST and update the cache... Let's hope for the best... :S" }
            ...
            permissionsResult = cacheService.getPermissions(guildId, userId)

            if (permissionsResult.missingRoles) { // bye
