...
class LavaReversoCommand(loritta: LorittaBot) : AbstractCommand(
    loritta,
    "lavareverse",
    listOf("lavareverso", "reverselava"),
    ...
    override suspend fun run(context: CommandContext, locale: BaseLocale) {
        if (context.args.isNotEmpty()) {
            ...
            val template = readImage(File(LorittaBot.ASSETS + "lavareverso.png")) // Template
            ...
            transform.rotate(
                0.436332,
                (firstImageCanvas.width / 2).toDouble(),
                (firstImageCanvas.height / 2).toDouble()
            )
            ...
            var font = Font.createFont(0, File(LorittaBot.ASSETS + "mavenpro-bold.ttf")).deriveFont(32F)
            graphics.font = font
            ImageUtils.drawCenteredString(
                graphics,
                "O chão " + (if (singular) "é" else "são") + " $joined",
                Rectangle(2, 2, 693, 100),
                font
            )
