    ...
    companion object {
        ...
        suspend fun queueTextChannelTopicUpdates(loritta: LorittaBot, guild: Guild, serverConfig: ServerConfig) {
            ...
            val validChannels = guild.textChannels.filter { channel ->
                ...
                memberCounterConfig != null && guild.selfMemberHasPermission(
                    channel,
                    Permission.ManageChannels
                ) && memberCounterConfig.topic.contains("{counter}")
            }

            val channelsThatWillBeChecked =
                validChannels.take(ServerPremiumPlans.getPlanFromValue(activeDonationValues).memberCounterCount)
        ...
        private suspend fun queueTextChannelTopicUpdate(
            ...
            val memberCounterPendingForUpdateMutex =
                memberCounterExecutingUpdatesMutexes.getOrPut(textChannel.idLong) { Mutex() }
            val memberCounterExecutingUpdateMutex =
                memberCounterExecutingUpdatesMutexes.getOrPut(textChannel.idLong) { Mutex() }
        ...
        private suspend fun updateTextChannelTopic(
            loritta: LorittaBot,
            guild: Guild,
            serverConfig: ServerConfig,
            textChannel: Channel,
            memberCounterConfig: MemberCounterChannelConfig
        ) {
    ...
    override fun onGuildMemberJoin(event: GuildMemberJoinEvent) {
        ...
        GlobalScope.launch(loritta.coroutineDispatcher) {
            try {
                ...
                val autoroleConfig =
                    serverConfig.getCachedOrRetreiveFromDatabase<AutoroleConfig?>(loritta, ServerConfig::autoroleConfig)
                val welcomerConfig =
                    serverConfig.getCachedOrRetreiveFromDatabase<WelcomerConfig?>(loritta, ServerConfig::welcomerConfig)
                ...
                val mute = loritta.newSuspendedTransaction {
                    Mute.find { (Mutes.guildId eq event.guild.idLong) and (Mutes.userId eq event.member.user.idLong) }
                        .firstOrNull()
                }
                ...
                if (mute != null) {
                    ...
                    val locale = loritta.localeManager.getLocaleById(serverConfig.localeId)
                    val muteRole = MuteCommand.getMutedRole(
                        loritta,
                        event.guild,
                        loritta.localeManager.getLocaleById(serverConfig.localeId)
                    ) ?: return@launch
