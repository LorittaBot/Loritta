...
object AdminUtils {
    ...
    suspend fun sendConfirmationMessage(
        ...
        val str =
            context.locale["${LOCALE_PREFIX}.readyToPunish", context.locale["commands.command.$type.punishName"], users.joinToString { it.asMention }, users.joinToString { it.asTag }, users.joinToString { it.id }]
    ...
    suspend fun sendSuccessfullyPunishedMessage(
        context: CommandContext,
        reason: String,
        sendDiscordReportAdvise: Boolean
    ) {
        val replies = mutableListOf(
            LorittaReply(
    ...
    fun createPunishmentEmbedBuilderSentViaDirectMessage(
        guild: Guild,
        locale: BaseLocale,
        punisher: User,
        ...
        embed.addField(
            "\uD83D\uDC6E ${locale["$LOCALE_PREFIX.punishedBy"]}",
            punisher.name + "#" + punisher.discriminator,
            false
        )
    ...
    fun createPunishmentMessageSentViaDirectMessage(
        guild: Guild,
        locale: BaseLocale,
        punisher: User,
        punishmentAction: String,
        ...
        return createPunishmentEmbedBuilderSentViaDirectMessage(
            guild,
            locale,
            punisher,
            punishmentAction,
    ...
    suspend fun getPunishmentForMessage(
        loritta: LorittaBot,
        settings: ModerationConfigSettings,
        guild: Guild,
    ...
    data class AdministrationOptions(
        val reason: String,
