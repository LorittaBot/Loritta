...
class DiscordCommandContext(
    ...
    override suspend fun sendImage(
        ...
        fileName: String,
        content: String
    ): net.perfectdreams.loritta.morenitta.api.entities.Message {
    ...
    suspend fun sendFile(
        file: File,
        fileName: String,
        content: String = this.getUserMention(true),
        embed: DeviousEmbed? = null
    ): DiscordMessage {
        return DiscordMessage(
            discordMessage.channel.sendMessage(
                MessageBuilder()
    ...
    override suspend fun imageUrl(argument: Int, searchPreviousMessages: Int): String? {
        if (this.args.size > argument) { // Primeiro iremos verificar se existe uma imagem no argumento especificado
            ...
            try {
                var unicodeEmoji =
                    LorittaUtils.toUnicode(this.args[argument].codePointAt(0)) // Vamos usar codepoints porque emojis
        ...
        if (!this.isPrivateChannel && this.guild.selfMemberHasPermission(
                this.discordMessage.textChannel,
        ...
        if (searchPreviousMessages > 0 && !this.isPrivateChannel && guild.selfMemberHasPermission(
    ...
    override suspend fun explain() {
        ...
        val embed = EmbedBuilder()
            ...
            .setFooter(
                "${user.name + "#" + user.discriminator} • ${command.category.getLocalizedName(locale)}",
        ...
        if (command is DiscordCommand) {
            if (command.botRequiredPermissions.isNotEmpty() || command.userRequiredPermissions.isNotEmpty()) {
                ...
                if (command.userRequiredPermissions.isNotEmpty()) {
                    field += "\uD83D\uDC81 ${
                        locale["commands.explain.youNeedToHavePermission", command.userRequiredPermissions.toSet()
                            .toLocalized()?.joinToString(", ", transform = { "`${i18nContext.get(it)}`" })]
                    }\n"
                }
                if (command.botRequiredPermissions.isNotEmpty()) {
                    field += "<:loritta:331179879582269451> ${
                        locale["commands.explain.loriNeedToHavePermission", command.botRequiredPermissions.toSet()
                            .toLocalized()?.joinToString(", ", transform = { "`${i18nContext.get(it)}`" })]
                    }\n"
                }
                embed.addField(
