...
object TretaNewsGenerator {
    ...
    fun generate(loritta: LorittaBot, guild: Guild, usr1: User, usr2: User): GeneratedTretaNews {
        ...
        run {
            ...
            graphics.font = font
            graphics.drawString(
                LorittaBot.RANDOM.nextInt(2, 10).toString() + ":" + LorittaBot.RANDOM.nextInt(10, 60),
                211,
                122 + font.size
            )
        }
        ...
        val texto =
            LorittaBot.RANDOM.nextInt(1, 24).toString() + " horas atrás • " + df.format(views) + " visualizações"

        checkY = ImageUtils.drawTextWrap(loritta, texto, 244, checkY, 655, 0, g2d.fontMetrics, g2d)

        checkY += 18

        var descricao = TretaNewsGenerator.randomDescriptions[LorittaBot.RANDOM.nextInt(
            0,
            TretaNewsGenerator.randomDescriptions.size - 1
        )]
        ...
        return GeneratedTretaNews(
            originalTitle,
            views,
            LorittaBot.RANDOM.nextInt(30000, 100000),
            LorittaBot.RANDOM.nextInt(1000, 10000),
            youtube
        )
    }
