...
class PedraPapelTesouraCommand(loritta: LorittaBot) : AbstractCommand(
    loritta,
    "jankenpon",
    listOf("pedrapapeltesoura", "ppt"),
    ...
    override suspend fun run(context: CommandContext, locale: BaseLocale) {
        ...
        if (context.args.isNotEmpty()) {
            ...
            if (janken != null) {
                ...
                context.reply(
                    LorittaReply(
                        message = context.locale["commands.command.rockpaperscissors.chosen", janken.emoji, opponent.emoji],
                        prefix = prefix
                    ),
                ...
                if (playerValue.equals("jesus", ignoreCase = true)) {
                    ...
                    context.reply(
                        LorittaReply(
                            message = context.locale["commands.command.rockpaperscissors.chosen", jesus, jesus],
                            prefix = "\uD83C\uDFF3"
                        ),
                        LorittaReply(message = fancy, mentionUser = false)
                    )
                    ...
                    context.reply(
                        LorittaReply(
                            message = context.locale["commands.command.rockpaperscissors.chosen", "\uD83D\uDCA9", jesus],
                            prefix = "\uD83C\uDFF4"
                        ),
                        LorittaReply(message = fancy, mentionUser = false)
                    )
