...
suspend fun ApplicationCall.respondJson(json: JsonElement, status: HttpStatusCode? = null) =
    this.respondText(ContentType.Application.Json, status) {
        gson.toJson(json)
    }

suspend fun ApplicationCall.respondJson(json: JsonNode, status: HttpStatusCode? = null) =
    this.respondText(ContentType.Application.Json, status) {
        Constants.JSON_MAPPER.writeValueAsString(json)
    }

suspend fun ApplicationCall.respondJson(json: kotlinx.serialization.json.JsonElement, status: HttpStatusCode? = null) =
    this.respondText(ContentType.Application.Json, status) {
        json.toString()
    }

suspend fun ApplicationCall.respondJson(json: String, status: HttpStatusCode? = null) =
    this.respondText(ContentType.Application.Json, status) {
        json
    }

suspend fun ApplicationCall.respondHtml(html: String, status: HttpStatusCode? = null) =
    this.respondText(ContentType.Text.Html, status) { html }
 ...
 */
val ApplicationRequest.trueIp: String
    get() {
 ...
 */
val ApplicationRequest.urlQueryString: String
    get() {
