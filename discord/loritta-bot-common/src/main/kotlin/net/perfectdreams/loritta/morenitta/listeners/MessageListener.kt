...
class MessageListener(val loritta: LorittaBot) : ListenerAdapter() {
    companion object {
        ...
        private val unavailableMessages = Collections.newSetFromMap(
            Caffeine.newBuilder().expireAfterWrite(15L, TimeUnit.MINUTES).build<Long, Boolean>().asMap()
        )
    }
    ...
    private val messageEditedModules = mutableListOf(automodModule)
    ...
    override fun onMessageReceived(event: MessageReceivedEvent) {
        ...
        if (event.message.type != MessageType.Default && event.message.type != MessageType.Reply) // Existem vários tipos de mensagens no Discord, mas apenas estamos interessados nas mensagens padrões de texto
            return
        ...
        loritta.launchMessageJob(event) {
            try {
                ...
                if (event.guild != null && member != null) {
                    ...
                    if (isMentioningOnlyMe(event.message.contentRaw)) {
                        ...
                        if (lorittaUser.hasPermission(LorittaPermission.IGNORE_COMMANDS)) {
                            ...
                            if (serverConfig.blacklistedChannels.contains(channel.idLong) && !lorittaUser.hasPermission(
                    ...
                    if (autoroleConfig != null && autoroleConfig.enabled && autoroleConfig.giveOnlyAfterMessageWasSent && event.guild.selfMemberHasPermission(
                    ...
                    loritta.messageInteractionCache.values.toMutableList().forEach {
                        ...
                        if (it.guildId == event.guild.idLong && it.channelId == channel.idLong) {
                    ...
                    start = System.nanoTime()
                    if (channel.canTalk() && serverConfig.warnOnUnknownCommand) {
                    ...
                    val lorittaUser =
                        LorittaUser(loritta, event.author, EnumSet.noneOf(LorittaPermission::class.java), profile)
                    val currentLocale = loritta.newSuspendedTransaction {
                ...
                logger.error(
                    "[${event.guild?.name}] Erro ao processar mensagem de ${event.author.name} (${event.author.id}) - ${event.message.contentRaw}",
                    e
                )
            }
        }
    ...
    override fun onMessageUpdate(event: MessageUpdateEvent) {
        ...
        if (event.channel.type == ChannelType.GuildText) { // Mensagens em canais de texto
            val guild = event.guild
            val channel = event.channel
                ...
                GlobalScope.launch(loritta.coroutineDispatcher) {
                    ...
                    val lorittaUser = GuildLorittaUser(
                        ...
                        LorittaUser.convertRolePermissionsMapToMemberPermissionList(
                            member,
                            serverConfig.getOrLoadGuildRolesLorittaPermissions(loritta, guild)
                        ),
                        lorittaProfile
                    )
     ...
     */
    suspend fun checkCommandsAndDispatch(
        lorittaMessageEvent: LorittaMessageEvent,
        ...
        if (lorittaMessageEvent.channel.type == ChannelType.GuildText && !lorittaMessageEvent.channel.canTalk())
            return false
        ...
        val startsWithLorittaMention =
            firstLabel == "<@${loritta.config.loritta.discord.applicationId.toString()}>" || firstLabel == "<@!${loritta.config.loritta.discord.applicationId.toString()}>"
        ...
        if (startsWithCommandPrefix || startsWithLorittaMention) {
            ...
            // Executar comandos
            var start = System.nanoTime()
            if (loritta.commandMap.dispatch(
                    lorittaMessageEvent,
                    rawArguments,
                    serverConfig,
                    locale,
            ...
            start = System.nanoTime()
            if (loritta.legacyCommandManager.matches(
                    lorittaMessageEvent,
                    rawArguments,
                    serverConfig,
                    locale,
     ...
     */
    fun isMentioningOnlyMe(contentRaw: String): Boolean =
        contentRaw.replace("!", "").trim() == "<@${loritta.config.loritta.discord.applicationId.toString()}>"
