	...
	companion object {
		...
		suspend fun checkIfUserCanPayout(loritta: LorittaBot, userIdentification: TemmieDiscordAuth.UserIdentification, ip: String): Int {
			...
			val lastReceivedDailyAt = loritta.newSuspendedTransaction {
				net.perfectdreams.loritta.morenitta.tables.Dailies.select { Dailies.receivedById eq userIdentification.id.toLong() and (Dailies.receivedAt greaterEq todayAtMidnight) }.orderBy(Dailies.receivedAt, SortOrder.DESC)
					.map {
