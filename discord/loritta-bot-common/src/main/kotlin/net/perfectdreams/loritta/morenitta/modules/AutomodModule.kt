...
class AutomodModule(val loritta: LorittaBot) : MessageReceivedModule {
    companion object {
        val MESSAGES =
            Caffeine.newBuilder().expireAfterWrite(5L, TimeUnit.MINUTES).build<String, Queue<Message>>().asMap()
    ...
    override suspend fun handle(
        ...
        if (ANTIRAID_ENABLED && (loritta.config.loritta.antiRaidIds.contains(Snowflake(event.channel.id))) && loritta.config.loritta.environment == EnvironmentType.CANARY) {
            val messages =
                MESSAGES.getOrPut(event.channel.id) { Queues.synchronizedQueue(EvictingQueue.create<Message>(50)) }
            ...
            fun calculateRaidingPercentage(wrapper: Message): Double {
                ...
                messageLoop@ for ((index, message) in messages.reversed().withIndex()) {
                    ...
                    if (message.contentRaw.isNotBlank()) {
                        ...
                        val threshold = LevenshteinDistance.getDefaultInstance()
                            .apply(compareContent.toLowerCase(), content.toLowerCase())
                        ...
                        val similarMessageScore =
                            distanceMultiplier * AutomodModule.SIMILAR_MESSAGE_MULTIPLIER * (Math.max(
                                0,
                                AutomodModule.SIMILARITY_THRESHOLD - threshold
                            ))
                ...
                val similarSameAuthorScore =
                    AutomodModule.SIMILAR_SAME_AUTHOR_MESSAGE_MULTIPLIER * verySimilarMessages.size
                ...
                val newAccountScore = AutomodModule.FRESH_ACCOUNT_DISCORD_MULTIPLIER * Math.max(
                    0,
                    AutomodModule.FRESH_ACCOUNT_TIMEOUT - (System.currentTimeMillis() - wrapper.author.timeCreated.toInstant()
                        .toEpochMilli())
                )
                ...
                if (member != null) {
                    val recentlyJoinedScore = AutomodModule.FRESH_ACCOUNT_JOINED_MULTIPLIER * Math.max(
                        0,
                        AutomodModule.FRESH_ACCOUNT_TIMEOUT - (System.currentTimeMillis() - member.timeJoined.toInstant()
                            .toEpochMilli())
                    )
            ...
            if (raidingPercentage >= BAN_THRESHOLD) {
                ...
                val selfMember = event.guild.retrieveSelfMember()
                ...
                mutex.withLock {
                    ...
                    for (storedMessage in messages) {
                        ...
                        if (percentage >= BAN_THRESHOLD) {
                            ...
                            if (selfMember.canInteract(event.member!!)) {
                                ...
                                BanCommand.ban(
                                    ...
                                    "Tentativa de Raid (Spam/Flood)! Que feio, para que fazer isto? Vá procurar algo melhor para fazer em vez de incomodar outros servidores. ᕙ(⇀‸↼‶)ᕗ",
                                    false,
                                    7
                                )
                            }
