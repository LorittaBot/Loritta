...
class UserInfoCommand(loritta: LorittaBot) : AbstractCommand(
    loritta,
    "userinfo",
    listOf("memberinfo"),
    ...
    fun getEmbedBase(context: CommandContext, user: User, member: Member?): EmbedBuilder {
        return EmbedBuilder().apply {
            ...
            if (addBadgesToField)
                addField(
                    "\uD83D\uDCDB ${context.locale["commands.command.userinfo.badges"]}",
                    getBadges(user).joinToString(""),
                    true
                )
    ...
    suspend fun showQuickGlanceInfo(message: Message?, context: CommandContext, user: User, member: Member?): Message {
        ...
        embed.apply {
            addField(
                "\uD83D\uDD16 ${context.locale["commands.command.userinfo.discordTag"]}",
            ...
            addField(
                "\uD83D\uDCC5 ${context.locale["commands.command.userinfo.accountCreated"]}",
                DateUtils.formatDateWithRelativeFromNowAndAbsoluteDifference(user.timeCreated, context.locale),
                true
            )
            if (member != null) {
                addField(
                    "\uD83C\uDF1F ${context.locale["commands.command.userinfo.accountJoined"]}",
                    DateUtils.formatDateWithRelativeFromNowAndAbsoluteDifference(member.timeJoined, context.locale),
                    true
                )
                ...
                if (timeBoosted != null)
                    addField(
                        "${Emotes.LORI_NITRO_BOOST} ${context.locale["commands.command.userinfo.boostingSince"]}",
                        DateUtils.formatDateWithRelativeFromNowAndAbsoluteDifference(timeBoosted, context.locale),
                        true
                    )
            }
    ...
    suspend fun showExtendedInfo(message: Message?, context: CommandContext, user: User, member: Member?): Message {
        ...
        embed.apply {
            if (member != null) {
                ...
                addField(
                    "\uD83D\uDCBC " + context.locale["commands.command.userinfo.roles"] + " (${member.roles.size})",
                    if (roles.isNotEmpty()) roles.substringIfNeeded(0 until 1024) else context.locale["commands.command.userinfo.noRoles"] + " \uD83D\uDE2D",
                    true
                )
