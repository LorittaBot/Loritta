...
class ShipCommand(loritta: LorittaBot) :
    AbstractCommand(loritta, "ship", listOf("shippar"), net.perfectdreams.loritta.common.commands.CommandCategory.FUN) {
    ...
    override suspend fun run(context: CommandContext, locale: BaseLocale) {
        ...
        if (user1Name != null && user2Name != null && user1Name.isNotEmpty() && user2Name.isNotEmpty()) {
            ...
            if (1 > user2Name.compareTo(user1Name)) {
                ...
                name2 =
                    reversedMentionedUsers[1].substring(reversedMentionedUsers[1].length / 2..reversedMentionedUsers[1].length - 1)
            ...
            if (user1 != null && user2 != null) {
                ...
                if ((user1.id == loritta.config.loritta.discord.applicationId.toString() || user2.id == loritta.config.loritta.discord.applicationId.toString()) && (!loritta.isOwner(
                        user1.id
                    ) && !loritta.isOwner(user2.id))
                ) {
            ...
            val avatar1Old =
                user1AvatarUrl?.let { LorittaUtils.downloadImage(loritta, it) } ?: Constants.DEFAULT_DISCORD_BLUE_AVATAR
            val avatar2Old =
                user2AvatarUrl?.let { LorittaUtils.downloadImage(loritta, it) } ?: Constants.DEFAULT_DISCORD_BLUE_AVATAR
