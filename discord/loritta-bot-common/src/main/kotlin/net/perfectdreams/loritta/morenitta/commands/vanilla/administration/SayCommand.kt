...
class SayCommand(loritta: LorittaBot) : AbstractCommand(
    loritta,
    "say",
    listOf("falar"),
        ...
        if (context.rawArgs.isNotEmpty()) {
            ...
            val channel = if (isEditMode) editMessage!!.channel else if (args.size >= 2) {
                if (channelIdOrMessageLink.startsWith("<#") && channelIdOrMessageLink.endsWith(">")) {
                    try {
                        val ch = context.guild.getTextChannelById(
                            channelIdOrMessageLink.substring(
                                2,
                                channelIdOrMessageLink.length - 1
                            )
                        )
                        args = args.remove(0)
                        ch
            ...
            if (channel.type == ChannelType.GuildText) { // Caso seja text channel...
                ...
                if (context.config.blacklistedChannels.contains(channel.idLong) && !context.lorittaUser.hasPermission(
