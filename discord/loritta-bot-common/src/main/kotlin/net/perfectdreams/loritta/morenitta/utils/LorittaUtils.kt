...
object LorittaUtils {
    ...
    suspend fun canUploadFiles(context: CommandContext): Boolean {
        if (!context.isPrivateChannel && !context.guild.selfMemberHasPermission(
    ...
    suspend fun isGuildOwnerBanned(loritta: LorittaBot, executorProfile: Profile?, guild: Guild): Boolean {
        val ownerProfile =
            if (guild.ownerIdLong == executorProfile?.id?.value) executorProfile else loritta.getLorittaProfileAsync(
                guild.ownerIdLong
            )
