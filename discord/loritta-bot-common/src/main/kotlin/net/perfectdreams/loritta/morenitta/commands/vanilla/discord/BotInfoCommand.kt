...
class BotInfoCommand(loritta: LorittaBot) :
    AbstractCommand(loritta, "botinfo", category = net.perfectdreams.loritta.common.commands.CommandCategory.DISCORD) {
    ...
    override suspend fun run(context: CommandContext, locale: BaseLocale) {
        ...
        embed.setAuthor(
            "${context.locale["commands.command.botinfo.title"]} 💁",
            loritta.config.loritta.website.url,
            "${loritta.config.loritta.website.url}assets/img/loritta_gabizinha_v1.png"
        )
        ...
        embed.addField(
            "\uD83C\uDF80 ${context.locale["website.donate.title"]}",
            "${loritta.config.loritta.website.url}donate",
            true
        )
        embed.addField(
            "<:loritta:331179879582269451> ${context.locale["website.jumbotron.addMe"]}",
            "${loritta.config.loritta.website.url}dashboard",
            true
        )
        embed.addField(
            "<:lori_ok_hand:426183783008698391> ${context.locale["modules.sectionNames.commands"]}",
            "${loritta.config.loritta.website.url}commands",
            true
        )
        embed.addField(
            "\uD83D\uDC81 ${context.locale["website.support.title"]}",
        ...
        embed.addField(
            "<:instagram:552841049660325908> Instagram",
            "[@lorittabot](https://instagram.com/lorittabot/)",
            true
        )
        ...
        embed.setFooter(
            "${locale["commands.command.botinfo.lorittaCreatedBy"]} - https://mrpowergamerbr.com/",
            loritta.lorittaShards.retrieveUserById(123170274651668480L)!!.effectiveAvatarUrl
        )
