...
class TempoCommand(loritta: LorittaBot) : AbstractCommand(
    loritta,
    "weather",
    listOf("tempo", "previsão", "previsao"),
    ...
    override suspend fun run(context: CommandContext, locale: BaseLocale) {
        if (context.args.isNotEmpty()) {
            ...
            val cidadeResponse = HttpRequest.get(
                "http://api.openweathermap.org/data/2.5/forecast?q=" + URLEncoder.encode(
                    cidade,
                    "UTF-8"
                ) + "&units=metric&lang=pt&APPID=" + loritta.config.loritta.openWeatherMap.key
            ).body()
            ...
            if (cidadeJsonResponse.get("cod").asString == "200") { // Nós encontramos alguma coisa?
                ...
                val countryShort =
                    if (cidadeJsonResponse["city"].obj.has("country")) cidadeJsonResponse.get("city").asJsonObject.get("country").asString else realCityName
                var icon = ""
                ...
                embed.addField(
                    "🌡 ${context.locale["commands.command.weather.temperature"]}",
                    "**${context.locale["commands.command.weather.current"]}: **$now ºC\n**${context.locale["commands.command.weather.max"]}: **$max ºC\n**${context.locale["commands.command.weather.min"]}: **$min ºC",
                    true
                )
