...
class AmizadeCommand(loritta: LorittaBot) : AbstractCommand(
    loritta,
    "friendship",
    listOf("amizade"),
    ...
    override suspend fun run(context: CommandContext, locale: BaseLocale) {
        if (context.message.mentionedUsers.size == 2) {
            ...
            val avatar =
                LorittaUtils.downloadImage(loritta, context.userHandle.getEffectiveAvatarUrl(ImageFormat.PNG, 128))
            ...
            val graphics =
                template.graphics.enableFontAntiAliasing() // É necessário usar Graphics2D para usar gradients
            ...
            ImageUtils.drawCenteredStringOutlined(
                graphics,
                locale["commands.command.friendship.ended"],
            ...
            ImageUtils.drawCenteredStringOutlined(
                graphics,
                "${locale["commands.command.friendship.now"]} " + user2.name,
                Rectangle(0, 100, 400, 110),
                font
            )
            ImageUtils.drawCenteredStringOutlined(
                graphics,
                locale["commands.command.friendship.isMy"],
            ...
            ImageUtils.drawCenteredStringOutlined(
                graphics,
                locale["commands.command.friendship.bestFriend"],
