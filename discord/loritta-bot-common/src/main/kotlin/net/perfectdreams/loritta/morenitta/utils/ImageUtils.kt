...
fun Graphics.drawText(loritta: LorittaBot, text: String, x: Int, y: Int, maxX: Int? = null) {
    ...
    for (c in textToBeDrawn.toCharArray()) {
        ...
        if (!this.font.canDisplay(c)) {
            ...
            if (emoteImage != null) {
                this.drawImage(
                    emoteImage.getScaledInstance(this.font.size, this.font.size, BufferedImage.SCALE_SMOOTH),
                    currentX,
                    currentY - this.font.size + 1,
                    null
                )
...
object ImageUtils {
    val emotes =
        CacheBuilder.newBuilder().expireAfterAccess(1, TimeUnit.HOURS).build<String, Optional<BufferedImage>>().asMap()

    fun getTwitterEmojiUrlId(emoji: String) =
        emoji.codePoints().toList().joinToString(separator = "-") { LorittaUtils.toUnicode(it).substring(2) }

    fun getTwitterEmoji(loritta: LorittaBot, text: String, index: Int): BufferedImage? {
        try {
            val imageUrl = "https://twemoji.maxcdn.com/2/72x72/" + LorittaUtils.toUnicode(text.codePointAt(index - 1))
                .substring(2) + ".png"
            try {
            ...
            if (!graphics.font.canDisplay(c)) {
                ...
                if (emoteImage != null) {
                    graphics.drawImage(
                        ...
                        ), currentX, currentY - graphics.font.size + 1, null
                    )
                    currentX += fontMetrics.maxAdvance
                }
                continue
        ...
        for (str in split) {
            ...
            for (c in str.toCharArray()) { // E agora nós iremos printar todos os chars
                ...
                if (!graphics.font.canDisplay(c)) {
                    ...
                    if (emoteImage != null) {
                        graphics.drawImage(
                            emoteImage.getScaledInstance(width, width, BufferedImage.SCALE_SMOOTH),
                            currentX,
                            currentY - width,
                            null
                        )
