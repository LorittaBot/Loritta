...
class SpigotMcCommand(loritta: LorittaBot) : AbstractCommand(
    loritta,
    "spigotmc",
    ...
    override suspend fun run(context: CommandContext, locale: BaseLocale) {
        if (context.args.isNotEmpty()) {
            ...
            val response =
                HttpRequest.get("https://api.spiget.org/v2/search/resources/${URLEncoder.encode(query, "UTF-8")}")
            ...
            if (json.isJsonObject) {
                ...
                if (array.size() == 1) {
                    ...
                    context.sendMessage(
                        ...
                        createResourceEmbed(context, resourceId, locale).build()
                    )
                } else {
                    var format = ""
    ...
    fun createResourceEmbed(context: CommandContext, resourceId: String, locale: BaseLocale): EmbedBuilder {
        ...
        val author =
            JsonParser.parseString(HttpRequest.get("https://api.spiget.org/v2/resources/$resourceId/author").body())

        embed.setAuthor(author["name"].string, null, "https://www.spigotmc.org/${author["icon"]["url"].string}")
        embed.setTitle(
            "<:spigotmc:375314413357629440> ${resource.name}",
        ...
        if (resource.contributors.isNotEmpty()) embed.addField(
            context.locale["commands.command.spigotmc.contributors"],
            resource.contributors,
            true
        )
        embed.addField(context.locale["commands.command.spigotmc.downloads"], resource.downloads.toString(), true)
        if (resource.testedVersions.isNotEmpty()) embed.addField(
            context.locale["commands.command.spigotmc.testedVersions"],
            resource.testedVersions.joinToString(separator = ", "),
            true
        )
        ...
        embed.addField(
            context.locale["commands.command.spigotmc.released"],
            releaseInstant.atOffset(ZoneOffset.UTC).humanize(locale),
            true
        )
        embed.addField(
            context.locale["commands.command.spigotmc.lastUpdated"],
            updateInstant.atOffset(ZoneOffset.UTC).humanize(locale),
            true
        )

        embed.addField(
            context.locale["commands.command.spigotmc.download"],
            "https://www.spigotmc.org/${resource.downloadLink}",
            true
        )
