...
object MiscUtils {
    ...
    suspend fun verifyAccount(
        ...
        val email =
            userIdentification.email ?: return AccountCheckResult.NOT_VERIFIED // Sem email == não verificado (?)
        ...
        if (!status.canAccess) {
            when (status) {
                ...
                MiscUtils.AccountCheckResult.NOT_VERIFIED -> {
                    ...
                    jsonObject("reason" to "Missing !canAccess result! ${status.name}")
                )
            }
        }
    }
    ...
    fun checkRecaptcha(serverToken: String, clientToken: String): Boolean {
        val body =
            HttpRequest.get("https://www.google.com/recaptcha/api/siteverify?secret=${serverToken}&response=$clientToken")
                .body()
