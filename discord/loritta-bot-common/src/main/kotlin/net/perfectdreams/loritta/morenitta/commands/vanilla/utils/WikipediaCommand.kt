...
class WikipediaCommand(loritta: LorittaBot) : AbstractCommand(
    loritta,
    "wikipedia",
    listOf("wiki"),
    ...
    override suspend fun run(context: CommandContext, locale: BaseLocale) {
        if (context.args.isNotEmpty()) {
            ...
            try {
                ...
                val wikipediaResponse = HttpRequest.get(
                    "https://" + languageId + ".wikipedia.org/w/api.php?format=json&action=query&prop=extracts|info&inprop=url&redirects=1&exintro=&explaintext=&titles=" + URLEncoder.encode(
                        query,
                        "UTF-8"
                    )
                ).body()
                ...
                if (entryWikiContent.key == "-1") { // -1 = Nenhuma página encontrada
                    ...
                    val embed = EmbedBuilder()
                        ...
                        .setDescription(
                            if (pageExtract.length > 512) pageExtract.substring(
                                0,
                                509
                            ) + "..." else pageExtract
                        )
