...
class PostTransferBalanceRoute(loritta: LorittaBot) :
    RequiresAPIAuthenticationRoute(loritta, "/api/v1/loritta/transfer-balance") {
    companion object {
    ...
    override suspend fun onAuthenticatedRequest(call: ApplicationCall) {
        ...
        mutex.withLock {
            ...
            val lastReceiverDailyAt = loritta.newSuspendedTransaction {
                Dailies.select { Dailies.receivedById eq receiverId and (Dailies.receivedAt greaterEq todayAtMidnight) }
                    .orderBy(Dailies.receivedAt, SortOrder.DESC)
                    .firstOrNull()
            }

            val lastGiverDailyAt = loritta.newSuspendedTransaction {
                Dailies.select { Dailies.receivedById eq giverId and (Dailies.receivedAt greaterEq todayAtMidnight) }
                    .orderBy(Dailies.receivedAt, SortOrder.DESC)
                    .firstOrNull()
            }
            ...
            if (lastReceiverDailyAt != null && lastGiverDailyAt != null) {
                ...
                if (receiverDailyIp == giverDailyIp) {
                    ...
                    val sameIpDaily = loritta.newSuspendedTransaction {
                        Dailies.select { Dailies.ip eq lastReceiverDailyAt[Dailies.ip] and (Dailies.receivedAt greaterEq todayAtMidnight) }
                            .orderBy(Dailies.receivedAt, SortOrder.DESC)
                            .toList()
                    }
