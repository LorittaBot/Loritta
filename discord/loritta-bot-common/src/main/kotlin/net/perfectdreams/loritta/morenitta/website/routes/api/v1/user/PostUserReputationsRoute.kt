...
class PostUserReputationsRoute(loritta: LorittaBot) :
    RequiresAPIDiscordLoginRoute(loritta, "/api/v1/users/{userId}/reputation") {
    companion object {
        private val logger = KotlinLogging.logger {}
        private val mutex = Mutex()

        fun sendReputationToCluster(
            loritta: LorittaBot,
            guildId: String,
            channelId: String,
            giverId: String,
            receiverId: String,
            reputationCount: Long
        ) {
        ...
        suspend fun sendReputationReceivedMessage(
            loritta: LorittaBot,
            guildId: String,
            channelId: String,
            giverId: String,
            ...
            if (guildId.isValidSnowflake() && channelId.isValidSnowflake()) {
                ...
                if (channel != null) {
                    ...
                    if (!channel.guild.retrieveSelfMember()
                            .hasPermission(channel, Permission.UseExternalEmojis, Permission.EmbedLinks)
                    ) // Permissões
                        return
                    ...
                    ) // O usuário não pode enviar comandos no canal
                        return
                    ...
                    val reply = LorittaReply(
                        locale[
                                ...
                                receiverSettings.gender.getPersonalPronoun(
                                    locale,
