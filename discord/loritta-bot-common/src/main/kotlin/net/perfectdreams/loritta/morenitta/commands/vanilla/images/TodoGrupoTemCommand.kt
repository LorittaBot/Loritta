...
class TodoGrupoTemCommand(loritta: LorittaBot) : AbstractCommand(
    loritta,
    "everygrouphas",
    listOf("todogrupotem"),
    ...
    override suspend fun run(context: CommandContext, locale: BaseLocale) {
        val bi =
            readImage(File(LorittaBot.ASSETS + context.locale["commands.command.everygrouphas.file"])) // Primeiro iremos carregar o nosso template
        ...
        ImageUtils.drawCenteredStringOutlined(
            baseGraph,
            locale["commands.command.everygrouphas.everygrouphas"],
            Rectangle(0, 0, 366, 22),
            font
        )
        ...
        for (aux in 5 downTo 0) {
            ...
            val avatarImg = LorittaUtils.downloadImage(loritta, member.getEffectiveAvatarUrl(ImageFormat.PNG, 128))!!
                .getScaledInstance(122, 122, Image.SCALE_SMOOTH)
