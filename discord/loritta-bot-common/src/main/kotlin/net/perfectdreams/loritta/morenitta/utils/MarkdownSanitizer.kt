...
class MarkdownSanitizer {
     ...
     */
    fun withStrategy(strategy: SanitizationStrategy): MarkdownSanitizer {
    ...
    private fun applyStrategy(region: Int, @Nonnull seq: String, @Nonnull builder: StringBuilder) {
        ...
        var token: String = tokens.get(region)
     ...
     */
    fun compute(sequence: String): String {
        val builder = StringBuilder()
        val end = handleQuote(sequence, false)
        ...
        while (i < sequence.length) {
            ...
            if (nextRegion == NORMAL) {
                if (sequence[i] == '\n' && i + 1 < sequence.length) {
                    val result = handleQuote(sequence.substring(i + 1), true)
    ...
    private fun handleQuote(@Nonnull sequence: String, newline: Boolean): String? {
    ...
    companion object {
         ...
         * @see .withIgnored
         */
        fun sanitize(sequence: String, strategy: SanitizationStrategy): String {
         ...
         * @return The string with escaped markdown
         */
        fun escape(sequence: String, single: Boolean): String {
