...
class InviteInfoCommand(loritta: LorittaBot) : AbstractCommand(
    loritta,
    "inviteinfo",
    ...
    override suspend fun run(context: CommandContext, locale: BaseLocale) {
        ...
        if (inviteId != null) {
            ...
            val inviteBody =
                HttpRequest.get("https://canary.discordapp.com/api/v6/invite/${inviteId.encodeToUrl()}?with_counts=true")
            ...
            if (code.asJsonPrimitive.isNumber && code.int == 10006) {
                ...
                embed.addField(
                    "\uD83D\uDC6E ${locale["commands.command.serverinfo.verificationLevel"]}",
                    verificationLevel.toString(),
                    true
                ) // ID da Guild
                embed.addField(
                    "\uD83D\uDC65 ${locale["commands.command.serverinfo.members"]}",
                    "\uD83D\uDC81 **${locale["commands.command.inviteinfo.active"]}:** ${approxPresenceCount}\n\uD83D\uDE34 **Offline:** $approxMemberCount",
                    true
                )

                if (features.size() == 0) {
                    embed.addField(
                        "✨ ${locale["commands.command.serverinfo.features"]}",
                        locale["commands.command.inviteinfo.none"],
                        true
                    ) // ID da Guild
                } else {
                    embed.addField(
                        "✨ ${locale["commands.command.serverinfo.features"]}",
                        features.joinToString(", ", transform = { it.string }),
                        true
                    ) // ID da Guild
                }
                ...
                embed.addField(
                    "\uD83D\uDDE3 ${locale["commands.command.inviteinfo.channelInvite"]}",
                    "`#${channel["name"].string}` (${channel["id"].string})",
                    true
                )
                ...
                if (inviter != null) {
                    ...
                    embed.addField(
                        "\uD83D\uDC4B ${locale["commands.command.inviteinfo.whoInvited"]}",
