...
class SadRealityExecutor(loritta: LorittaBot, val client: GabrielaImageServerClient) :
    CinnamonSlashCommandExecutor(loritta) {
    inner class Options : LocalizedApplicationCommandOptions(loritta) {
        val user1 = optionalUser(
            "user1",
            SadRealityCommand.I18N_PREFIX.Options.User1.Text(SadRealityCommand.I18N_PREFIX.Slot.TheGuyYouLike.Female)
        )
        val user2 = optionalUser(
            "user2",
            SadRealityCommand.I18N_PREFIX.Options.User2.Text(SadRealityCommand.I18N_PREFIX.Slot.TheFather.Male.LovedGenderFemale)
        )
        val user3 = optionalUser(
            "user3",
            SadRealityCommand.I18N_PREFIX.Options.User3.Text(SadRealityCommand.I18N_PREFIX.Slot.TheBrother.Male.LovedGenderFemale)
        )
        val user4 = optionalUser(
            "user4",
            SadRealityCommand.I18N_PREFIX.Options.User4.Text(SadRealityCommand.I18N_PREFIX.Slot.TheFirstLover.Male.LovedGenderFemale)
        )
        val user5 = optionalUser(
            "user5",
            SadRealityCommand.I18N_PREFIX.Options.User5.Text(SadRealityCommand.I18N_PREFIX.Slot.TheBestFriend.Male.LovedGenderFemale)
        )
        val user6 = optionalUser(
            "user6",
            SadRealityCommand.I18N_PREFIX.Options.User6.Text(SadRealityCommand.I18N_PREFIX.Slot.You.Male)
        )
    }
    ...
    override suspend fun execute(context: ApplicationCommandContext, args: SlashCommandArguments) {
        ...
        val lovedGender = profileSettings[listOfUsers[0].id.toLong()]?.gender
            ?: Gender.FEMALE // The default is FEMALE for the loved gender
        ...
        val result = userAvatarCollage(3, 2) {
            if (listOfUsers[0].id == applicationId) {
                ...
                localizedSlot(
                    context.i18nContext,
                    listOfUsers[0],
                    LorittaColors.LorittaAqua.toJavaColor(),
                    SadRealityCommand.I18N_PREFIX.Slot.TheGuyYouLike.Loritta
                )
            } else {
