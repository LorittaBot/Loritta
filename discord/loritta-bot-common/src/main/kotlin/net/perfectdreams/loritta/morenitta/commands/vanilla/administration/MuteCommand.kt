...
class MuteCommand(loritta: LorittaBot) : AbstractCommand(
    loritta,
    "mute",
    listOf("mutar", "silenciar"),
            ...
            suspend fun punishUser(time: Long?) {
                ...
                message.onReactionAddByAuthor(context) {
                    if (it.reactionEmote.isEmote("✅") || it.reactionEmote.isEmote("\uD83D\uDE4A")) {
                        ...
                        for (member in members) {
                            val result =
                                muteUser(context, settings, member, time, locale, member.user, reason, isSilent)
    ...
    companion object {
        ...
        suspend fun muteUser(
            ...
            if (!isSilent) {
                if (settings.sendPunishmentViaDm && context.guild.isMember(user)) {
                    try {
                        val embed = AdminUtils.createPunishmentEmbedBuilderSentViaDirectMessage(
                            context.guild,
                            locale,
                            context.userHandle,
                            locale["commands.command.mute.punishAction"],
                            reason
                        )
            ...
            if (couldntEditChannels.isNotEmpty()) {
                context.reply(
                    LorittaReply(
                        context.locale["commands.command.mute.couldntEditChannel", couldntEditChannels.joinToString(
                            ", ",
        ...
        fun getMutedRole(loritta: LorittaBot, guild: Guild, locale: BaseLocale) =
            guild.getRolesByName(locale["$LOCALE_PREFIX.mute.roleName"], false).getOrNull(0)

        suspend fun spawnRoleRemovalThread(
            loritta: LorittaBot,
            guild: Guild,
            locale: BaseLocale,
            user: User,
            expiresAt: Long
        ) = spawnRoleRemovalThread(loritta, guild.idLong, locale, user.idLong, expiresAt)
