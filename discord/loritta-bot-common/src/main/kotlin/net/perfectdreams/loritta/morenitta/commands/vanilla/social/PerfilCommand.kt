...
class PerfilCommand(loritta: LorittaBot) : AbstractCommand(
    loritta,
    "profile",
    listOf("perfil"),
    ...
    override suspend fun run(context: CommandContext, locale: BaseLocale) {
        ...
        if (contextUser == null && context.args.isNotEmpty() && (context.args.first() == "shop" || context.args.first() == "loja")) {
            context.reply(
                LorittaReply(
                    context.locale["commands.command.profile.profileshop", "${loritta.config.loritta.website.url}user/@me/dashboard/profiles"],
                    Emotes.LORI_OWO
                )
            )
            return
        ...
        context.sendFile(
            result.image.inputStream(),
            "lori_profile.${result.imageFormat.extension}",
            "📝 **|** " + context.getAsMention(true) + context.locale["commands.command.profile.profile"]
