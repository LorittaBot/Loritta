...
class EditarXPCommand(loritta: LorittaBot) : AbstractCommand(
    loritta,
    "editxp",
    listOf("editarxp", "setxp"),
    category = net.perfectdreams.loritta.common.commands.CommandCategory.SOCIAL
) {
    ...
    override suspend fun run(context: CommandContext, locale: BaseLocale) {
        ...
        if (user != null && context.rawArgs.size == 2) {
            ...
            val userData =
                context.config.getUserData(loritta, user.idLong, context.guild.retrieveMemberOrNull(user) != null)
