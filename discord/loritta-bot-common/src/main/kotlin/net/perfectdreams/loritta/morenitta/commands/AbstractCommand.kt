...
abstract class AbstractCommand(
    ...
    var category: net.perfectdreams.loritta.common.commands.CommandCategory,
    var lorittaPermissions: List<LorittaPermission> = listOf(),
    ...
    suspend fun explain(context: CommandContext) {
        ...
        val embed = EmbedBuilder()
            ...
            .setFooter(
                "${user.name + "#" + user.discriminator} • ${this.category.getLocalizedName(locale)}",
        ...
        if (botPermissions.isNotEmpty() || discordPermissions.isNotEmpty()) {
            ...
            if (discordPermissions.isNotEmpty()) {
                field += "\uD83D\uDC81 ${
                    locale["commands.explain.youNeedToHavePermission", discordPermissions.toSet().toLocalized()
                        ?.joinToString(", ", transform = { "`${context.i18nContext.get(it)}`" })]
                }\n"
            }
            if (botPermissions.isNotEmpty()) {
                field += "<:loritta:331179879582269451> ${
                    locale["commands.explain.loriNeedToHavePermission", botPermissions.toSet().toLocalized()
                        ?.joinToString(", ", transform = { "`${context.i18nContext.get(it)}`" })]
                }\n"
            }
            embed.addField(
