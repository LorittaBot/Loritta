...
open class NostalgiaProfileCreator(loritta: LorittaBot, internalName: String, val folderName: String) :
    StaticProfileCreator(loritta, internalName) {
    class NostalgiaDarkProfileCreator(loritta: LorittaBot) : NostalgiaProfileCreator(loritta, "defaultDark", "dark")
    class NostalgiaBlurpleProfileCreator(loritta: LorittaBot) :
        NostalgiaProfileCreator(loritta, "defaultBlurple", "blurple")
    ...
    class NostalgiaPurpleProfileCreator(loritta: LorittaBot) :
        NostalgiaProfileCreator(loritta, "defaultPurple", "purple")

    class NostalgiaPinkProfileCreator(loritta: LorittaBot) : NostalgiaProfileCreator(loritta, "defaultPink", "pink")
    class NostalgiaYellowProfileCreator(loritta: LorittaBot) :
        NostalgiaProfileCreator(loritta, "defaultYellow", "yellow")

    class NostalgiaOrangeProfileCreator(loritta: LorittaBot) :
        NostalgiaProfileCreator(loritta, "defaultOrange", "orange")
    ...
    override suspend fun create(
        ...
        val avatar = LorittaUtils.downloadImage(loritta, user.avatarUrl)!!
            .getScaledInstance(152, 152, BufferedImage.SCALE_SMOOTH)
        ...
        if (guild != null) {
            val localProfile = loritta.newSuspendedTransaction {
                GuildProfile.find { (GuildProfiles.guildId eq guild.id.toLong()) and (GuildProfiles.userId eq user.id.toLong()) }
                    .firstOrNull()
            }
        ...
        if (marriage != null) {
            ...
            if (marriedWith != null) {
                ...
                ImageUtils.drawCenteredString(
                    graphics,
                    locale["profile.marriedWith"],
                    Rectangle(545, 108, 256, 14),
                    latoBold16
                )
                graphics.font = latoRegular20
                ImageUtils.drawCenteredString(
                    graphics,
                    marriedWith.name + "#" + marriedWith.discriminator,
                    Rectangle(545, 108 + 14, 256, 18),
                    latoRegular20
                )
                graphics.font = latoBold16
                ImageUtils.drawCenteredString(
                    graphics,
                    DateUtils.formatDateDiff(marriage.marriedSince, System.currentTimeMillis(), locale),
                    Rectangle(545, 108 + 14 + 18, 256, 14),
                    latoBold16
                )
            }
        }
