                    ...
                    if (storedMessage != null) {
                        ...
                        embed.setFooter(
                            WebhookEmbed.EmbedFooter(
                                locale["modules.eventLog.userID", user.id.toString()],
                                null
                            )
                        )
            ...
            if (eventLogConfig.enabled && eventLogConfig.messageDeleted) {
                ...
                if (textChannel != null && textChannel.canTalk()) {
                    val storedMessages = loritta.newSuspendedTransaction {
                        StoredMessage.find { StoredMessages.id inList event.messageIds.map { it.toLong() } }
                            .toMutableList()
                    }
                    ...
                    if (storedMessages.isNotEmpty()) {
                        ...
                        for (message in storedMessages) {
                            ...
                            val line =
                                "[${creationTime.format(DateUtils.PRETTY_DATE_FORMAT)}] (${message.authorId}) ${messageSentByUser?.name}#${messageSentByUser?.discriminator}: ${
                                    message.decryptContent(loritta)
                                }"
                            lines.add(line)
                        }
                        ...
                        val channelName =
                            event.guild.getTextChannelById(storedMessages.first().channelId)?.name ?: "unknown"
                            ...
                            WebhookMessageBuilder()
                                ...
                                .addFile(
                                    "deleted-${event.guild.name}-$channelName-${
                                        DateUtils.PRETTY_FILE_SAFE_UNDERSCORE_DATE_FORMAT.format(
                                            Instant.now()
                                        )
                                    }.log", targetStream
                                )
