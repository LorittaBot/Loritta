...
abstract class CommandContext(
    ...
    open suspend fun sendImage(image: Image, fileName: String = "image.png", content: String = getUserMention(true)) =
        message.channel.sendFile(image.toByteArray(), fileName, content)

    open suspend fun sendFile(byteArray: ByteArray, fileName: String, content: String = getUserMention(true)) =
        message.channel.sendFile(byteArray, fileName, content)
    ...
    abstract suspend fun image(
        argument: Int,
        searchPreviousMessages: Int = 25,
        createTextAsImageIfNotFound: Boolean = true
    ): Image?
     ...
     * @see CommandException
     */
    fun fail(message: String, prefix: String? = null): Nothing =
        throw CommandException(message, prefix ?: Emotes.LORI_CRYING.toString())
    ...
    suspend fun validate(user: User?, argumentIndex: Int = 0): User {
        if (user == null) {
            if (args.isEmpty())
                ...
                fail(
                    locale["commands.userDoesNotExist", "${args.getOrNull(argumentIndex)?.replace("`", "")}"],
