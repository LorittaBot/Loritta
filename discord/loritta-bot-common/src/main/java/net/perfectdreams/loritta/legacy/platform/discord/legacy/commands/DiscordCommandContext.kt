...
class DiscordCommandContext(
	...
	override suspend fun sendImage(image: Image, fileName: String, content: String): net.perfectdreams.loritta.legacy.api.entities.Message {
	...
	override suspend fun sendFile(byteArray: ByteArray, fileName: String, content: String): net.perfectdreams.loritta.legacy.api.entities.Message {
		return DiscordMessage(
				discordMessage.channel.sendMessage(LorittaMessage(content).content)
	...
	override suspend fun imageUrl(argument: Int, searchPreviousMessages: Int): String? {
		if (this.args.size > argument) { // Primeiro iremos verificar se existe uma imagem no argumento especificado
			...
			if (LorittaUtils.isValidUrl(link) && net.perfectdreams.loritta.legacy.utils.loritta.connectionManager.isTrusted(link)) {
			...
			for (embed in discordMessage.embeds) {
				if (embed.image != null && net.perfectdreams.loritta.legacy.utils.loritta.connectionManager.isTrusted(embed.image!!.url!!))
					return embed.image!!.url
			}
			for (attachment in discordMessage.attachments) {
				if (attachment.isImage && net.perfectdreams.loritta.legacy.utils.loritta.connectionManager.isTrusted(attachment.url))
