...
import net.perfectdreams.loritta.legacy.platform.discord.legacy.commands.DiscordCommand
import net.perfectdreams.loritta.legacy.serializable.CommandInfo
...
class GetCommandsRoute(val loritta: LorittaDiscord) : BaseRoute("/api/v1/loritta/commands/{localeId}") {
	override suspend fun onRequest(call: ApplicationCall) {
		...
		val commands = net.perfectdreams.loritta.legacy.utils.loritta.legacyCommandManager.commandMap.map {
			CommandInfo(
		...
		} + net.perfectdreams.loritta.legacy.utils.loritta.commandMap.commands.filter { !it.hideInHelp }.map {
