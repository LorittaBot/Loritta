...
import net.perfectdreams.loritta.legacy.utils.WebsiteVoteUtils
...
class PostDiscordBotsCallbackRoute(val loritta: LorittaDiscord) : BaseRoute("/api/v1/callbacks/discord-bots") {
	...
	override suspend fun onRequest(call: ApplicationCall) {
		...
		if (authorizationHeader != net.perfectdreams.loritta.legacy.utils.loritta.config.generalWebhook.webhookSecret) {
		...
		if (type == "upvote" || (type == "test" && net.perfectdreams.loritta.legacy.utils.loritta.config.isOwner(userId))) {
