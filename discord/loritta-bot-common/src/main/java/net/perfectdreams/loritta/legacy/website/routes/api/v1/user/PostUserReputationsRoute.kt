...
class PostUserReputationsRoute(loritta: LorittaDiscord) : RequiresAPIDiscordLoginRoute(loritta, "/api/v1/users/{userId}/reputation") {
	...
	override suspend fun onAuthenticatedRequest(call: ApplicationCall, discordAuth: TemmieDiscordAuth, userIdentification: LorittaJsonWebSession.UserIdentification) {
		...
		if (!MiscUtils.checkRecaptcha(net.perfectdreams.loritta.legacy.utils.loritta.config.googleRecaptcha.reputationToken, token))
			throw WebsiteAPIException(
					HttpStatusCode.Forbidden,
					WebsiteUtils.createErrorPayload(
		...
		mutex.withLock {
			...
			val donatorPaid = net.perfectdreams.loritta.legacy.utils.loritta.getActiveMoneyFromDonationsAsync(userIdentification.id.toLong())
			...
			if (chance(randomChance)) { // Lori é fofis e retribuiu reputações :eu_te_moido:
				GlobalScope.launch(net.perfectdreams.loritta.legacy.utils.loritta.coroutineDispatcher) {
