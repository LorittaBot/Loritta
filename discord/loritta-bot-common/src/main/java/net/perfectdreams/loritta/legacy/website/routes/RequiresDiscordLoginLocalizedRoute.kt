...
import net.perfectdreams.loritta.legacy.website.utils.extensions.toJson
import net.perfectdreams.loritta.legacy.website.utils.extensions.toWebSessionIdentification
import net.perfectdreams.loritta.legacy.website.views.UserBannedView
			...
			if (code == null) {
				if (discordAuth == null) {
					if (call.request.header("User-Agent") == Constants.DISCORD_CRAWLER_USER_AGENT) {
						...
						redirect(net.perfectdreams.loritta.legacy.utils.loritta.discordInstanceConfig.discord.authorizationUrl + "&state=${Base64.getEncoder().encodeToString(state.toString().toByteArray()).encodeToUrl()}", false)
					}
				}
			} else {
				...
				if (guildId != null) {
					...
					var tries = 0
					val maxGuildTries = net.perfectdreams.loritta.legacy.utils.loritta.config.loritta.website.maxGuildTries
					...
					while (!guildFound && maxGuildTries > tries) {
						...
						if (guild != null) {
							...
							val serverConfig = net.perfectdreams.loritta.legacy.utils.loritta.getOrCreateServerConfig(guild.idLong)
									...
									if (!user.isBot && (member.hasPermission(Permission.MANAGE_SERVER) || member.hasPermission(Permission.ADMINISTRATOR))) {
										...
										if (guildOwner != null) {
											...
											val message = locale.getList(
												...
												user.asMention,
												guild.name,
												net.perfectdreams.loritta.legacy.utils.loritta.instanceConfig.loritta.website.url + "commands",
												net.perfectdreams.loritta.legacy.utils.loritta.instanceConfig.loritta.website.url + "guild/${guild.id}/configure/",
												net.perfectdreams.loritta.legacy.utils.loritta.instanceConfig.loritta.website.url + "guidelines",
												net.perfectdreams.loritta.legacy.utils.loritta.instanceConfig.loritta.website.url + "donate",
												net.perfectdreams.loritta.legacy.utils.loritta.instanceConfig.loritta.website.url + "support",
												Emotes.LORI_PAT,
												Emotes.LORI_NICE,
												Emotes.LORI_HEART,
												Emotes.LORI_COFFEE,
