...
object WebsiteAssetsHashes {
	...
	fun getAssetHash(assetName: String): String {
		return if (websiteFileHashes.containsKey(assetName)) {
			...
			val md5 = DigestUtils.md5Hex(File("${net.perfectdreams.loritta.legacy.website.LorittaWebsite.FOLDER}/static/v2/$assetName").inputStream())
	...
	fun getLegacyAssetHash(assetName: String): String {
		return if (legacyWebsiteFileHashes.containsKey(assetName)) {
			...
			val md5 = DigestUtils.md5Hex(File("${net.perfectdreams.loritta.legacy.website.LorittaWebsite.FOLDER}/static/$assetName").inputStream())
