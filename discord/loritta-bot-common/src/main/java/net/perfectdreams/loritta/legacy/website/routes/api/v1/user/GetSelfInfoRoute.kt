...
class GetSelfInfoRoute(val loritta: LorittaDiscord) : BaseRoute("/api/v1/users/@me/{sections?}") {
	...
	override suspend fun onRequest(call: ApplicationCall) {
		...
		if (sections == null) {
			...
			val profile = net.perfectdreams.loritta.legacy.utils.loritta.getLorittaProfile(userIdentification.id)
			...
			val profile by lazy { net.perfectdreams.loritta.legacy.LorittaLauncher.loritta.getOrCreateLorittaProfile(userIdentification.id) }
