...
import net.perfectdreams.loritta.legacy.commands.vanilla.economy.PagarCommand
...
class PostTransferBalanceRoute(loritta: LorittaDiscord) : RequiresAPIAuthenticationRoute(loritta, "/api/v1/loritta/transfer-balance") {
	...
	override suspend fun onAuthenticatedRequest(call: ApplicationCall) {
		...
		mutex.withLock {
			val receiverProfile = net.perfectdreams.loritta.legacy.utils.loritta.getOrCreateLorittaProfile(receiverId)
			val giverProfile = net.perfectdreams.loritta.legacy.utils.loritta.getOrCreateLorittaProfile(giverId)
