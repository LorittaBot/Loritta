...
import net.perfectdreams.loritta.legacy.utils.TakingMoreSonhosThanAllowedException
...
class Profile(id: EntityID<Long>) : Entity<Long>(id) {
	...
	suspend fun canGetDaily(): Pair<Boolean, Long> {
		val receivedDailyAt = loritta.newSuspendedTransaction {
			net.perfectdreams.loritta.legacy.tables.Dailies.select { Dailies.receivedById eq userId }
