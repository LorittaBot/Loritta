...
import net.perfectdreams.loritta.legacy.Loritta.Companion.RANDOM
import net.perfectdreams.loritta.legacy.dao.GuildProfile
	...
	companion object {
		...
		suspend fun checkIfUserCanPayout(userIdentification: TemmieDiscordAuth.UserIdentification, ip: String): Int {
			...
			val lastReceivedDailyAt = loritta.newSuspendedTransaction {
				net.perfectdreams.loritta.legacy.tables.Dailies.select { Dailies.receivedById eq userIdentification.id.toLong() and (Dailies.receivedAt greaterEq todayAtMidnight) }.orderBy(Dailies.receivedAt, SortOrder.DESC)
					.map {
			...
			val sameIpDailyAt = loritta.newSuspendedTransaction {
				net.perfectdreams.loritta.legacy.tables.Dailies.select { Dailies.ip eq ip and (Dailies.receivedAt greaterEq todayAtMidnight) }
			...
			val sameIpDailyOneHourAgoAt = loritta.newSuspendedTransaction {
				net.perfectdreams.loritta.legacy.tables.Dailies.select { Dailies.ip eq ip and (Dailies.receivedAt greaterEq nowOneHourAgo) }
					.orderBy(Dailies.receivedAt, SortOrder.DESC)
					.map {
						it[Dailies.receivedAt]
					}
			...
			if (lastReceivedDailyAt.isNotEmpty() || sameIpDailyOneHourAgoAt.isNotEmpty()) {
				if (!net.perfectdreams.loritta.legacy.utils.loritta.config.isOwner(userIdentification.id.toLong())) {
