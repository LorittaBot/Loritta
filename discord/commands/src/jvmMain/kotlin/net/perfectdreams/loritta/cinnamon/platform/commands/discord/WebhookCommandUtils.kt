...
object WebhookCommandUtils {
    ...
    suspend fun editMessageViaWebhook(
        ...
        val matchResult = WebhookUtils.webhookRegex.find(webhookUrl) ?: context.failEphemerally(context.i18nContext.get(WebhookCommand.I18N_PREFIX.InvalidWebhookUrl))
    ...
    private fun validateMessageLength(context: ApplicationCommandContext, optionalContent: Optional<String?>) {
        optionalContent.value?.length?.let {
            if (it > DiscordResourceLimits.Message.Length)
                context.failEphemerally(context.i18nContext.get(WebhookCommand.I18N_PREFIX.MessageTooBig(DiscordResourceLimits.Message.Length)))
        }
    }
