...
class SetXpExecutor : SlashCommandExecutor() {
    companion object : SlashCommandExecutorDeclaration(SetXpExecutor::class) {
        object Options : ApplicationCommandOptions() {
            val amount = integer("amount", EditXpCommand.I18N_PREFIX.Set.Options.Amount).register()
            val user = optionalUser("user", EditXpCommand.I18N_PREFIX.Set.Options.User).register()
        }
    ...
    override suspend fun execute(context: ApplicationCommandContext, args: SlashCommandArguments) {
        if (context !is GuildApplicationCommandContext) context.fail {
        ...
        if (amount > 0) context.failEphemerally {
            styled(
                context.i18nContext.get(EditXpCommand.I18N_PREFIX.InvalidRange), Emotes.Error
            )
        }

        val userProfile = context.loritta.services.servers.getOrCreateGuildUserProfile(
            UserId(user.id.value), context.guildId.value
        )
