...
class NotificationsCommand(loritta: LorittaCinnamon) : CinnamonSlashCommandDeclarationWrapper(loritta) {
    companion object {
    ...
    override fun declaration() = slashCommand("notifications", CommandCategory.UTILS, I18N_PREFIX.Description) {
        subcommand("list", I18N_PREFIX.List.Description) {
            executor = NotificationsListExecutor(loritta)
        }

        subcommand("view", I18N_PREFIX.View.Description) {
            executor = NotificationsViewExecutor(loritta)
        }
    }
}