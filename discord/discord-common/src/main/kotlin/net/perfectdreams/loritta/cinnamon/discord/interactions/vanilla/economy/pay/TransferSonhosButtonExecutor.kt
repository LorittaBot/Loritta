...
class TransferSonhosButtonExecutor(
    ...
    override suspend fun onClick(user: User, context: ComponentContext) {
        ...
        val result = loritta.services.transaction {
            val dataFromDatabase = loritta.services.interactionsData._getInteractionData(decodedGenericInteractionData.interactionDataId) ?: return@transaction Result.DataIsNotPresent // Data is not present! Maybe it expired or it was already processed
            ...
            val receiverProfile = loritta.services.users._getOrCreateUserProfile(UserId(decoded.userId))
            val giverProfile = loritta.services.users._getOrCreateUserProfile(UserId(decoded.sourceId))
            ...
            loritta.services.interactionsData._deleteInteractionData(decodedGenericInteractionData.interactionDataId)

            // Get the profiles again
            val updatedReceiverProfile = loritta.services.users._getOrCreateUserProfile(UserId(decoded.userId))
            val updatedGiverProfile = loritta.services.users._getOrCreateUserProfile(UserId(decoded.sourceId))

            val receiverRanking = if (updatedReceiverProfile.money != 0L) loritta.services.sonhos._getSonhosRankPositionBySonhos(updatedReceiverProfile.money) else null
            val giverRanking = if (updatedGiverProfile.money != 0L) loritta.services.sonhos._getSonhosRankPositionBySonhos(updatedGiverProfile.money) else null
