...
import net.perfectdreams.loritta.website.backend.utils.imgSrcSet
...
class DonateView(
                                            ...
                                            if (needsToLogin) {
                                                a(href = url) {
                                                    div(classes = "discord-button primary") {
                                                        ...
                                                        LorittaWebsiteBackend.svgIconManager.gift.apply(this)
    ...
    inner class DonationReward(val name: String, val minimumDonation: Double, val doNotDisplayInPlans: Boolean, val callback: TD.(Double) -> Unit = { column ->
        if (column >= minimumDonation) {
            LorittaWebsiteBackend.svgIconManager.check.apply(this)
        } else {
            LorittaWebsiteBackend.svgIconManager.xmark.apply(this)
        }
    })
}