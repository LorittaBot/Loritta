...
class HomeView(settings: WebRenderSettings, locale: BaseLocale) : NavbarView(settings, locale) {
    ...
    override fun DIV.generateContent() {
        div {
            ...
            div {
                ...
                img(src = "${settings.websiteUrl}/v2/assets/img/loritta_v2.png") {}
                img(src = "${settings.websiteUrl}/v2/assets/img/loritta_v2_dark.png", classes = "dark-sweater-pose") {}
                img(src = "${settings.websiteUrl}/v2/assets/img/loritta_v2_blink.png", classes = "blinking-pose") {}
                img(src = "${settings.websiteUrl}/v2/assets/img/loritta_v2_blush.png", classes = "blushing-pose") {}
            }
            ...
            div(classes = "right-side-text") {
                ...
                div(classes = "buttons") {
                    div {
                        a(classes = "add-me button pink has-shadow is-big", href = settings.addBotUrl) {
                            img(classes = "lori-happy", src = "${settings.websiteUrl}$versionPrefix/assets/img/lori_happy.gif")
        ...
        div(classes = "odd-wrapper") {
            generateAdOrSponsor(listOf(), 0, "8349707350", "Loritta v2 Below Header", true)
            generateAdOrSponsor(listOf(), 1, "8349707350", "Loritta v2 Below Header", false)
            ...
            div(classes = "media") {
                ...
                div(classes = "media-figure") {
                    imgSrcSet(
                            "${settings.websiteUrl}${versionPrefix}/assets/img/home/",
                            "lori_gabi.png",
        ...
        trust(locale)
        funnyCommands(locale, settings.websiteUrl)
        chitChat(locale, settings.websiteUrl)
        // music(locale, settings.websiteUrl)
        moderation(locale, settings.websiteUrl)
        notify(locale)
        customization(locale)
        community(locale)
        muchMore(locale)
        makeItAwesome(settings, locale)
        thankYou(locale)
    }
}