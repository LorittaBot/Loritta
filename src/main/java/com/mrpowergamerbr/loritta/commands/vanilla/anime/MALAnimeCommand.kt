...
class MALAnimeCommand : AbstractCommand("malanime", category = CommandCategory.ANIME) {
	override fun getDescription(locale: BaseLocale): String {
		return locale["MALANIME_Description"]
	}

	override fun getDetailedUsage(): Map<String, String> {
		return mapOf("query" to "Anime que você deseja procurar")
	}

	override fun getExtendedExamples(): Map<String, String> {
		return mapOf(
				"Dragon Ball Super" to "Procura por \"Dragon Ball Super\" no MyAnimeList",
				"Sonic" to "Procura por \"Sonic\" no MyAnimeList",
				"Parappa the Rapper" to "Procura por \"Parappa the Rapper\" no MyAnimeList"
		)
	}
	...
	override fun run(context: CommandContext, locale: BaseLocale) {
		if (context.args.isNotEmpty()) {
			...
			val websiteResponse = HttpRequest.get("https://myanimelist.net/api/anime/search.xml?q=${query.encodeToUrl()}")
			...
			val obj = JSON_PARSER.parse(jsonPrettyPrintString)["anime"]["entry"]
			...
			if (!obj.isJsonArray) {
				...
				val embed = EmbedBuilder().apply {
					...
					for ((index, anime) in animes.withIndex()) {
						...
						description += "${Constants.INDEXES[index]} [$title](https://myanimelist.net/anime/${anime["id"].string})\n"
					}
	...
	fun displayAnime(obj: JsonObject, locale: BaseLocale): EmbedBuilder {
		return EmbedBuilder().apply {
			...
			val episodes = obj["episodes"].string
			...
			setTitle("\uD83D\uDCFA $title", "https://myanimelist.net/anime/${obj["id"].string}")
			...
			if (episodes != "0")
				addField("\uD83C\uDFAC ${locale["MALANIME_Episodes"]}", episodes, true)
