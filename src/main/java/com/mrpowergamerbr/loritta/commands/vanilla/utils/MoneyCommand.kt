...
class MoneyCommand : CommandBase("money") {
	...
	override fun run(context: CommandContext) {
		if (context.args.size >= 2) {
			...
			val fixerResponse = JSON_PARSER.parse(response).asJsonObject // Base
			val validCurrResponse = JSON_PARSER.parse(validCurrenciesResponse).asJsonObject // Valid Currencies
