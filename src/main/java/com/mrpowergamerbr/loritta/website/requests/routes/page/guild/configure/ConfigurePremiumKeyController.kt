...
@Path("/:localeId/guild/:guildId/configure/premium")
class ConfigurePremiumKeyController {
	@GET
	...
	fun handle(req: Request, res: Response, @Local variables: MutableMap<String, Any?>, @Local guild: Guild, @Local newServerConfig: ServerConfig, @Local serverConfig: MongoServerConfig, @Local userIdentification: SimpleUserIdentification): String {
		variables["saveType"] = "premium"

		variables["premium_json"] = gson.toJson(WebsiteUtils.transformToDashboardConfigurationJson(userIdentification, guild, newServerConfig, serverConfig))

		return evaluate("configure_premium.html", variables)
	}
}