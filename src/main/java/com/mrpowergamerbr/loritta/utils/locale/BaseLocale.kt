 ...
 */
open class BaseLocale(val localeId: String) {
	companion object {
		@JvmStatic
	...
	operator fun get(fileAndKey: String, vararg arguments: Any?): String {
		if (fileAndKey.contains(":")) {
			// Novo jeito
			val split = fileAndKey.split(":")
			val file = split[0]
			val key = split[1]

			// TODO: result.map { MessageFormat.format(it.toString(), *args) }
			return getWithType<String>(file, key).f(*arguments)
		} else {
			if (!strings.containsKey(fileAndKey)) {
				logger.warn { "Missing translation key! $fileAndKey" }
				return fileAndKey
			}
			return strings[fileAndKey]!!.f(*arguments)
		}
	}
	...
	fun <T> format(vararg args: Any?, block: BaseLocale.() -> T): T {
		...
		return when (result) {
			is String -> "Requires Updates!" as T
			is List<*> -> listOf("Requires Updates!") as T
			else -> throw UnsupportedOperationException("Can't parse $result in BaseLocale!")
		}
	}

	fun <T> getWithType(fileAndKey: String): T {
		val split = fileAndKey.split(":")
		val file = split[0]
		val key = split[1]

		return getWithType(file, key)
	}

	fun <T> getWithType(fileName: String, key: String): T {
		val map = parsedYamls[fileName] ?: throw RuntimeException("Missing $fileName.yml for the $localeId language!")

		val splittedKey = key.split(".")

		var currentEntry: Any? = map
		for ((index, partOfTheKey) in splittedKey.withIndex()) {
			if (currentEntry !is Map<*, *>) {
				throw RuntimeException("Key $key is invalid! $partOfTheKey is not a Map so we can't traverse it!")
			}
			currentEntry = (currentEntry as Map<String, Any?>)[key]
		}
