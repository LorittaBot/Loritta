				...
				try {
					...
					if (partnerRole != null) {
						...
						for (partnerServerConfig in partnerServerConfigs) {
							...
							val usersWithPermission = runBlocking { lorittaShards.queryCluster(cluster, "/api/v1/guilds/${partnerServerConfig.guildId}/users-with-any-permission/${Permission.ADMINISTRATOR},${Permission.MANAGE_SERVER},${Permission.MANAGE_ROLES}").await() }
