...
class PerfilCommand : AbstractCommand("profile", listOf("perfil"), CommandCategory.SOCIAL) {
	...
	override suspend fun run(context: CommandContext, locale: LegacyBaseLocale) {
		...
		if (userProfile.userId == loritta.discordConfig.discord.clientId.toLong()) {
			aboutMe = locale.toNewLocale()["commands.social.profile.lorittaDescription"]
		}
		...
		if (images.size == 1) {
			context.sendFile(images.first(), "lori_profile.png", "📝 **|** " + context.getAsMention(true) + context.legacyLocale.toNewLocale()["commands.social.profile.profile"]) // E agora envie o arquivo
		} else {
			// Montar a GIF
			...
			context.sendFile(outputFile, "lori_profile.gif", "📝 **|** " + context.getAsMention(true) + context.legacyLocale.toNewLocale()["commands.social.profile.profile"]) // E agora envie o arquivo
		}
	}
}