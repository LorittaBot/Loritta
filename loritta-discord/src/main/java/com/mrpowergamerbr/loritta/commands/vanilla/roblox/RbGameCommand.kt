...
class RbGameCommand : AbstractCommand("rbgame", listOf("rbjogo", "rbgameinfo"), CommandCategory.ROBLOX) {
	...
	override suspend fun run(context: CommandContext,locale: LegacyBaseLocale) {
		if (context.args.isNotEmpty()) {
			...
			embed.addField("\uD83D\uDC4D ${locale.toNewLocale()["commands.images.tretanews.like"]}", upvotes, true)
			embed.addField("\uD83D\uDC4E ${locale.toNewLocale()["commands.images.tretanews.dislikes"]}", downvotes, true)
			...
			embed.addField("\uD83C\uDF1F ${locale.toNewLocale()["commands.discord.serverinfo.createdAt"]}", created, true)
