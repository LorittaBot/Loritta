...
class ServerBanSupportTicketSystem(
    jda: JDA,
    ...
    override val ticketCreatedMessage: InlineMessage<*>.(User, I18nContext) -> Unit = { sender, language ->
        content = (
                listOf(
                    LorittaReply(
                        language.get(I18nKeysData.Tickets.ThreadCreated.ReadyRoleMention("<@&${staffRoleId}>")),
                        "<:lori_coffee:727631176432484473>",
                        mentionUser = true
                    ),
                    LorittaReply(
                        "Este ticket é para caso você esteja banido do SERVIDOR NO DISCORD do $inviteUrl",
                        mentionUser = false
                    )
                )
                )
